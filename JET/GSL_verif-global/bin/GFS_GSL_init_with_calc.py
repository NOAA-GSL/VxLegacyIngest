...
def main():
    ...
    for fcst_len in forecast_leads:
        src_file_date_str = time.strftime('%Y%m%d')
        src_file_hour_str = time.strftime('%H')
        src_file = 'gfs.%s/%s/atmos/gfs.t%sz.pgrb2.0p25.f%s' % (src_file_date_str,src_file_hour_str,src_file_hour_str,fcst_len)
