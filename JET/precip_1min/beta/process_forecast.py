...
def unprocessed(model,valid_time,fcst_len):
    ...
    for each_file in all_files:
        if each_file.find(".PCP_1h_data.loaded") > 0:
            #print each_file
  ...
  if True:
    ...
    if True:
        if reprocess or unprocessed(model,valid_time,fcst_len):
            if model.find("HRRR_GSD") >= 0:
                (proj,good,PCP_1h) =  read_hrrr(model,valid_time,fcst_len)
            elif model.find("RAP") >= 0:
                (good,PCP_1h,lats,lons) =  read_rap(model,valid_time,fcst_len)
            elif model.find("NAVGEM") >= 0:
                (good,PCP_1h,lats,lons) =  read_NAVGEM_HIWPP_4(model,valid_time,fcst_len)
            if good:
                start_stats = time.clock()
                write_station_values(proj,stations,model,valid_time,fcst_len,PCP_1h)
