...
unless($result) {
    ...
    $dbh->do($query);
    $query = "create table $sums_table0 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    $query = "create table ruc_ua_sums2.${data_source}_Areg1 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    $query = "create table ruc_ua_sums2.${data_source}_Areg2 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    $query = "create table ruc_ua_sums2.${data_source}_Areg5 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    $query = "create table ruc_ua_sums2.${data_source}_Areg6 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    $query = "create table ruc_ua_sums2.${data_source}_Areg13 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
   $query = "create table ruc_ua_sums2.${data_source}_Areg17 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
   $query = "create table ruc_ua_sums2.${data_source}_Areg18 like ruc_ua_sums2.RAP_Areg0";
    $dbh->do($query);
    my $today = POSIX::strftime("%d-%b-%Y",gmtime(time())); 
    $query = qq{insert into ruc_ua.dp_to_rh_calculator_per_model VALUES}.
...
;
	...
	if($desired_filename) {
		...
		while(<V>) {
		    ...
		    if(/sigma=0\.(\d+) /) {
			if($1 > $grib_nz) {
			    $grib_nz = $1 + 0; # remove any leading zeros
			}
		    } elsif(/hybrid lev (\d+)/) {
			if($1 > $grib_nz) {
			    $grib_nz = $1 + 0; # remove any leading zeros
		...
		my $time_line=sprintf("%s %2.2d     %2.2d      $month_name     $vyear\n",
