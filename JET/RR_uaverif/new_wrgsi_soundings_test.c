...
{
  FILE *input = NULL;
  int hydro = 0;		/* ALWAYS generate hydrometeor data here */
  ...
  int n_g2_fields = 2;
  char *out_filename;
  long int floats_read=0;
  long int floats_to_read=0;
  ...
  if(argc < 12) {
    printf("Usage: new_wrgsi_soundings.x model filename grib_type grid_type tmp_file station_file"
	   "alat1 elon1 elonv alattan dx nx ny nz hydro wind_rot_flag\n");
    exit(1);
  }
  model = argv[arg_i++];
  filename = argv[arg_i++];
  ...
  (void)sscanf(argv[arg_i++],"%i",&wind_rot_flag);
  ...
  if(grib_type == 1) {
    ...
    /* grib2 */
    snprintf(line,500,"./make_grib2_tmp_file_test.pl %s %s %d %d",
  ...
  printf("      3           %-19.19s   12     kt\n",
   ...
   for(i=0;i<icount;i++) {
	...
	(raob_data[i].clwmr != MISSING || raob_data[i].rwmr != MISSING ||
	 ...
	 /*fprintf(stderr,"igroupel for %s: %ld, %ld\n", # this was causing issues with output streams - JAH */
	 printf("igroupel for %s: %ld, %ld\n",
