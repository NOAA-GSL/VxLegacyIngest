...
main (int argc, char *argv[])
{
  ...
  if(argc < 12) {
    printf("Usage: iso_wrgsi_soundings.x model filename grib_type grid_type tmp_file station_file"
  ...
  icount = 0;			/* counts output levels */
   for(i=0;i<nz;i++) {
     ...
     /*
     printf("lev %d, press: %ld, Z: %.6g, T: %.2f, RH: %.2g, U: %.2g, V: %.2g, %.2g, %.2g\n",
	    i+1,pr,Z,T,RH,U,V,ICMR,GRMR);
     */
