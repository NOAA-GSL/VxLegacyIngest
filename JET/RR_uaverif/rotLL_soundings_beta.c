...
main (int argc, char *argv[])
{
  ...
  int mand_level[] = {10000,9250,8500,7000,5000,4000,3000,2500,1500,1000};
  int mand_levels = 10;
  ...
  time_t endClock;
  float endSecs;
  long int ticks_per_sec, tmp_count;
  char *model;
  char *station_file;
  ...
  long floats_to_read;
  long floats_read;
  long read_level, read_next_level;
  long size_test;
  ...
  /*  if(argc != 6) {
    printf("Usage: rotLL_soundings.x model filename grib_type tmp_file station_file hydro\n");
    exit(1);
    }*/
  model = argv[arg_i++];
  filename = argv[arg_i++];
  ...
  (void)sscanf(argv[arg_i++],"%d",&hydro);
  ...
  size_test = (long) sizeof(float)*nx*ny*nz*n_g3_fields; 

  printf("size test: %ld\n",size_test);

  g3 = (float *)malloc(size_test);

  if(g3 == NULL) {
    printf("not enough space for g3 %d %d %d\n",
	   nx,ny,nz);
    exit(1);
  }
  ...
  if(grib_type == 1) {
   ...
   /* grib2 */
    snprintf(line,500,"./make_grib2_tmp_file.pl %s %s %d %d",
    ...
    floats_to_read = ((long) n_g3_fields*nx*ny*nz) + (n_g2_fields*nx*ny);
    floats_read = fread(g3,sizeof(float),(long)n_g3_fields*nx*ny*nz,input);
    floats_read += fread(g2,sizeof(float),n_g2_fields*nx*ny,input);
    if(floats_read == floats_to_read) {
      printf("SUCCESS\n");
      printf("%ld floats read. Should have read %ld\n",
  ...
  icount = 0;			/* counts output levels */
   for(i=0;i<nz;i++) {
     field_index = 0;
     /*printf("grabbing variables at startx: %d starty: %d nz: %d \n",
           startx,starty,i);*/
     long read_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i + (long)nz*field_index++)));
     P =   g3[read_level];
     read_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i + (long)nz*field_index++)));
     Z =   g3[read_level];
     read_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i + (long)nz*field_index++)));
     T =   g3[read_level];
     read_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i + (long)nz*field_index++)));
     ...
     /*printf("lev %d, press: %.6g, Z: %.6g, T: %.2f, Q: %.2g, U: %.2g, V: %.2g\n",
       i+1,P,Z,T,Q,U,V);*/
	 ...
	 while (done == 0) {
       ...
       if(i < nz-1) {
	 field_index = 0;
         long read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 P1 =   g3[read_next_level];
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 Z1 =   g3[read_next_level];
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 T1 =   g3[read_next_level];
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 Q1 =   g3[read_next_level];
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 U1 =   g3[read_next_level];
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 V1 =   g3[read_next_level];
	 lnCLWMR1 = MISSING;
	 lnRWMR1 = MISSING;
	 lnSNMR1 = MISSING;
	 lnICMR1 = MISSING;
	 lnGRMR1 = MISSING;
         ...
         if(hydro == 1) {
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 CLWMR1 = g3[read_next_level];
	 if(CLWMR1 > 0) {
	   lnCLWMR1 = log(CLWMR1)*10;
	 }
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 RWMR1 = g3[read_next_level];
	 if(RWMR1 > 0) {
	   lnRWMR1 = log(RWMR1)*10;
	 }
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 SNMR1 = g3[read_next_level];
	 if(SNMR1 > 0) {
	   lnSNMR1 = log(SNMR1)*10;
	 }
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 ICMR1 = g3[read_next_level];
	 if(ICMR1 > 0) {
	   lnICMR1 = log(ICMR1)*10;
	 }
         read_next_level = (long) ((long)startx + (long)nx*(starty + (long)ny*(i+1 + (long)nz*field_index++)));
	 GRMR1 = g3[read_next_level];
	 if(GRMR1 > 0) {
