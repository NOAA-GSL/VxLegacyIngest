...
#    } elsif($data_source eq "RAPv3_EMC") {
	...
	$anal_dir = sprintf("/lfs1/BMC/amb-verif/SAR_FV3_GSD/%4d%2.2d%2.2d%2.2d/postprd",
                        $year+1900,$month+1,$mday,$hour);
 	$template = sprintf("HRRR.t%02dz.bgdawp%02d.tm%02d",
			    $hour,$desired_fcst_len,$hour);

    } elsif($data_source eq "SAR_FV3_GSD") {
	$anal_dir = sprintf("/lfs1/BMC/amb-verif/SAR_FV3_GSD/%4d%2.2d%2.2d%2.2d/postprd",
                        $year+1900,$month+1,$mday,$hour);
 	$template = sprintf("HRRR.t%02dz.bgrd3d%02d.tm%02d",
			    $hour,$desired_fcst_len,$hour);
	...
	$anal_dir =
	    sprintf("/lfs1/BMC/wrfruc/HRRRE/forecast/%4d%2.2d%2.2d%2.2d/postprd_mem0001",
		    $year+1900,$month+1,$mday,$hour);
        $template = sprintf("wrfnat_mem0001_%02d.grib2",$desired_fcst_len);
    } elsif($data_source eq "HRRRE_mem2") {
	$anal_dir =
	    sprintf("/lfs1/BMC/wrfruc/HRRRE/forecast/%4d%2.2d%2.2d%2.2d/postprd_mem0002",
		    $year+1900,$month+1,$mday,$hour);
        $template = sprintf("wrfnat_mem0002_%02d.grib2",$desired_fcst_len);
    } elsif($data_source eq "HRRRE_mem3") {
	$anal_dir =
	    sprintf("/lfs1/BMC/wrfruc/HRRRE/forecast/%4d%2.2d%2.2d%2.2d/postprd_mem0003",
		    $year+1900,$month+1,$mday,$hour);
    ...
    print("\nanal_dir is $anal_dir\n");

    unless($template) {
	$template = sprintf("wrfnat_rr_%02d.grib2",$desired_fcst_len);
	# RR file names
	if($data_source =~ /^HRRRret_AK/) {
	    $template = sprintf("wrfnat_hralaska_%02d.grib2",$desired_fcst_len);
	} elsif($data_source =~ /^RRnc/) {
	    ...
	    $template = sprintf("wrfnat_rr_%02d.grib2",$desired_fcst_len);
            ...
            if($data_source =~ /iso/) {
              $template = sprintf("RRFS_NA_13km.t%02dz.bgdawp%03d.tm00.grib2",$hour,$desired_fcst_len);
            } else {
              $template = sprintf("RRFS_NA_13km.t%02dz.bgrd3df%03d.tm00.grib2",$hour,$desired_fcst_len,$hour);
            }
        } elsif($data_source =~ /^RRFS_AKret/) {
            if($data_source =~ /iso/) {
              $template = sprintf("RRFS_AK.t%02dz.bgdawpf%03d.tm00.grib2",$hour,$desired_fcst_len);
            } else {
              $template = sprintf("RRFS_AK.t%02dz.bgrd3df%03d.tm00.grib2",$hour,$desired_fcst_len);
            }
