...
main (int argc, char *argv[])
{
  ...
  long mand_level[] = {1000,975,950,925,900,850,800,750,700,650,600,550,500,450,400,350,300,250,200,150,100,70,50,40,30,20,15,10,7,5,3,2,1};
  int mand_levels = 33;
...
/*     Z_array  = &g3[0 * nz * nx * ny];
     T_array  = &g3[1 * nz * nx * ny];
     RH_array = &g3[2 * nz * nx * ny];
     U_array  = &g3[3 * nz * nx * ny];
     V_array  = &g3[4 * nz * nx * ny];
*/
   for(i=0;i<mand_levels;i++) {
     field_index = 0;
...
/*     index = startx + (starty*nx)+ (i*nx*ny);
     Z = Z_array[index];
     T = T_array[index];
     RH = RH_array[index]/100;
     U = U_array[index];
     V = V_array[index];*/
     lnCLWMR = MISSING;
     lnRWMR = MISSING;
