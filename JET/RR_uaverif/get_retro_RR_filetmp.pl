...
#    } elsif($data_source eq "RAPv3_EMC") {
	...
	$anal_dir =
	    sprintf("RR_retro/$data_source/RRFS_conus_3km.%4d%2.2d%2.2d/%2.2d",
		    $year+1900,$month+1,$mday,$hour);
    }
	...
	if($data_source =~ /^HRRRret_AK/) {
	    ...
	    $template = sprintf("wrfnat_rr_%02d.grib2",$desired_fcst_len);
            ...
            if($data_source =~ /iso/) {
              $template = sprintf("RRFS_CONUS.t%02dz.bgdawpf0%02d.tm00.grib2",$hour,$desired_fcst_len);
            } else {
              $template = sprintf("RRFS_CONUS.t%02dz.bgrd3df0%02d.tm00.grib2",$hour,$desired_fcst_len);
            }
	} elsif($data_source eq "HRRR") {
	    $template = sprintf("wrfnat_hrconus_%02d.grib2",$desired_fcst_len);
