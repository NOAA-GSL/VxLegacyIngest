...
$dbh->do("use ruc_ua_sums3");
foreach my $region (@regions) {
#   $query = "create table ${data_source}_Areg$region like ${iso}RR1h_Areg0";
    my $sums_table = "${data_source}_Areg$region";
    $query = qq(show tables like "$sums_table");
    print "in sums db: $query\n";
    my $result = $dbh->selectrow_array($query);
    unless($result) {
       $query = "create table ${sums_table} like template";
       print "$query;\n";
       $dbh->do($query);
    }
}
...
foreach my $valid_time (@valid_times) {
	...
	if($desired_filename) {
		...
		print MAIL<<EOI
To: $recipients
From: RR_verification.processing
...
;
        ...
        my $file_size = -s $desired_filename;

        if($file_size == 0) {
          print "FILE EMPTY $data_source $fcst_len_for_db h fcst valid at $valid_date\n";
          next;
        }
...
#            if($data_source =~ /^rt_gfs/ ) {
#                $arg_unsafe = "/whome/rtfim/fv3_soundings/fv3_soundings_grib2.x ".
#                    "$data_source $desired_filename $station_file $valid_time $fcst_len_from_file";
#            }
	    if($data_source =~ /iso/ || $data_source eq "NAM_NEST_AK" || $data_source eq "NAM_NEST_HI" || $data_source eq "FV3_GFS_EMC" ) {
...
#	    }
		...
		$arg_unsafe = "$thisDir/rotLL_soundings_iso_beta.x ".
		    "$data_source $desired_filename $grib_type $grid_type $tmp_file $station_file $hydro";
              }
		...
		$arg_unsafe = "$thisDir/rotLL_soundings_beta.x ".
		    "$data_source $desired_filename $grib_type $grid_type $tmp_file $station_file $hydro";
            }
	    if($data_source =~ /^rt_gfs/ || $data_source =~ /^GFSv17/) {
	...
	if($reprocess == 0 &&
	   stats_generated($data_source,$valid_time,$fcst_len_for_db)) {
		...
		my $command = "~/java8/java -Xmx256m Verify3_beta dummy_dir $data_source ".
...
sub stats_generated($data_source,$valid_time,$fcst_len_for_db) {
    ...
    my $table = "";
    my $query = qq{
	show tables from ruc_ua_sums3
	where Tables_in_ruc_ua_sums3 regexp '^${data_source}_A?reg[[:digit:]]*\$'};
