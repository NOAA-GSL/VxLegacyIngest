...
use lib "/whome/amb-verif/RR_uaverif/";
use Time::Local;
...
# create raobs file
my $region = 7;			# HARDWIRED FOR Global 
...
;
	...
	if($desired_filename && $make_soundings) {
		...
		$grib_nz = 39;
		$hydro = 1;  # check if cloud, ice variable in grib file 
            }elsif($data_source =~ /^HRRR_AK_OPS_iso/) {
		$grib_nz = 39;
		$hydro = 1;  # check if cloud, ice variable in grib file 
	    ...
	    elsif($data_source =~ /NCO_gfsv16_test_iso/) {
		$grib_nz = 39;
		$hydro = 0;
	    }
	    ...
	    elsif($data_source eq "RAP_OPS_iso" || $data_source eq "RAP_OPS_WCOSS2_iso" || $data_source eq "HRRR_OPS_WCOSS2_a_iso" || $data_source eq "HRRR_AK_OPS_WCOSS2_iso") {
		$grib_nz = 39;
		$hydro = 1;
	    }
...
#		"$alat1 $elon1 $elonv $alattan $grib_dx $grib_nx $grib_ny $grib_nz";
	    ...
	    if($data_source =~ /iso/ || $data_source eq "NAM_NEST_AK" || $data_source eq "NAM_NEST_HI") {
