...
Sounding load_sounding(String directory, String data_source,String site,int wmoid,
  ...
  try {
	...
	date.leadingZero(end_date.getHour());
      ...
      //      Debug.println("Xue Sounding URL = "+soundingURL);
      urlc.connect();
	...
	key = urlc.getHeaderFieldKey(++i2);
      ...
      //       Debug.println("Xue sounding in if in get from bfin ");
      
    } else {
	//	       Debug.println("Xue sounding in else in get from DB ");
      in = get_from_database(stmt,data_source,site,date.getSQLDateTime(),fcst_len);
     }
	...
	//Debug.println("Xue souding line=in.readline: "+line);
	  ...
	  if(i==-1) {
		...
		short_type="Dn";
	      ...
	      } else if(maybe.equals("persistence")) {
		short_type="P";
...
public DataInputStream get_from_database(Statement stmt,
  ...
  if(fcst_len > -127) {
    ...
    fcst_len_clause= "and (fcst_len = -99 or fcst_len >= 0) order by "+fcst_len+"\n";
  }
  table = data_source+"_raob_soundings";
  query = "show tables like '"+table+"'";
  try {
    rs = stmt.executeQuery("use soundings");
