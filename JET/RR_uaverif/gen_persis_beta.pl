...
#SBATCH --mem=16G
#SBATCH -D .
#SBATCH -e /home/amb-verif/RR_uaverif/tmp/RAOB_persis.o%j
#SBATCH -o /home/amb-verif/RR_uaverif/tmp/RAOB_persis.e%j
#
use strict;
...
$ENV{DBI_DSN} = "DBI:mysql:ruc_ua_sums3:wolphin.fsl.noaa.gov";
...
unless($result) {
    ...
    $query = "create table $data_source like RUC13_template";
    print "$query;\n";
    $dbh->do($query);
...
;
	...
	$query = "create table ${data_source}_Areg$region like Atemplate";
	print "$query;\n";
	$dbh->do($query);
    }
}
...
foreach my $valid_time (@valid_times) {
	...
	if(raobs_loaded($valid_time)) {
	    ...
	    my $command = "java -Xmx256m VerifyPersis_beta dummy_dir $data_source ".
