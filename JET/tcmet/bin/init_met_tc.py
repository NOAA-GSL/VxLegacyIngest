...
for basin in ["al","ep","cp"]:
    ...
    for storm in all_storms[basin]:
        ...
        if today < enddate + timedelta(days=1):
            ...
            print("        Determining final adeck file")
            filematch = (adeck_dir + "/*.a" + basin + storm + year + ".dat")
            all_files = glob.glob(filematch)
            all_files = sorted(all_files, key = lambda x:x.split('.')[0].split('/')[-1])
            enddate_str = all_files[-1].split('.')[0].split('/')[-1]
            print("        Final adeck file issued on ", enddate_str)
            print("        Copying final adeck file to working dir: ", adeck_verif_dir)
            os.system("cp " + adeck_dir + "/" + enddate_str + "." + verif_adeck + " " +  adeck_verif_dir + "/" + enddate_str + "." + verif_adeck)
