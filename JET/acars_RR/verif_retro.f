...
c     Moninger, Jun-July 2014 - adapted for variable lambert cc grids
c     Moninger, july 2014 - adapted for grib2
...
**************************************************************************
      ...
      integer grib_type
      integer grid_type
      real alat1,elon1,elonv,alattan,dx
      integer nx,ny,nz
      character*80 arg

      call getarg(1,adate)
      call getarg(2,model)
      call getarg(3,grib_file)
      call getarg(4,arg)
      read(arg,*) grib_type
      call getarg(5,arg)
      read(arg,*) grid_type
      ...
      call getarg(7,arg)
      read(arg,*) alat1     ! lat_0_deg for grid_type 20, 21, or 22
      call getarg(8,arg)
      read(arg,*) elon1    ! lon_o_deg for grid_type 20, 21, or 22
      call getarg(9,arg)
      read(arg,*) elonv    ! lat_g_SW_deg for grid_type 20, 21, or 22
      call getarg(10,arg)
      read(arg,*) alattan ! lon_g_SW_deg for grid_type 20, 21, or 22
      call getarg(11,arg)
      read(arg,*) dx              ! dx in meters
      call getarg(12,arg)
      read(arg,*) nx
      call getarg(13,arg)
      read(arg,*) ny
      ...
      call verif5a (adate,model,grib_file,grib_type,grid_type,
     . output_file,alat1,elon1,elonv,alattan,dx,nx,ny,nz)
      end

      subroutine verif5a (adate,model,grib_file,grib_type,grid_type,
     . output_file,alat1,elon1,elonv,alattan,dx,nx,ny,nz)
...
c  indices of fields desired from agrib
      ...
      data g3_fields/7,11,52,33,34/ ! HGT,TMP,RH,UGRD,VGRD
      real g3(nx,ny,nz,n_g3_fields) 
c g2 is a dummy, because we don't want any sfc fields here.
      ...
      real fht(nx,ny,nz,numproj) !5 forecast projections for model fcsts
      real ft(nx,ny,nz,numproj) !5 forecast projections for model fcsts
      real frh(nx,ny,nz,numproj) !5 forecast projections for model fcsts
      real fu(nx,ny,nz,numproj)
      real fv(nx,ny,nz,numproj)
...
c
      ...
      integer grid_type
      real alat1,elon1,elonv,alattan,dx
      real lat_0_deg,lon_0_deg,lat_g_SW_deg,lon_g_SW_deg
      integer nx,ny,nz
      integer plev(nz)
c     data plev/1013,1000,975,950,925,900,875,850,825,800,
c     *               775,750,725,700,675,650,625,600,
c     *               575,550,525,500,475,450,425,400,
c     *               375,350,325,300,275,250,225,200,
c     *               175,150,125,100,75,50/

c****************************************************************************
      print *,'args are',adate,model,grib_file,grib_type,grid_type,output_file,alat1,elon1,elonv,alattan,dx,nx,ny,nz
      if (nz .eq. 38 .or. nz .eq. 40) then
         plev(1) = 1013
         do i = 2,nz
            plev(i) = 1050 - i*25
c     print *,i,plev(i)
         enddo
      else if(nz .eq. 37 .or. nz .eq. 39) then
         plev(1) = 1000;
         do i = 2,nz
            plev(i) = 1025 - i*25
         enddo
      end if
      rotcon = sin(alattan*0.0174533)
      print*,'sine of ',alattan,rotcon
...
c      write (*,*) 'degribbing ',trim(grib_file)
      if (grib_type == 1) then
         ret = agrib(nz,n_g3_fields,g3_fields,
     *        n_g2_fields,g2_fields,g3,g2,-1,-1,
     *        trim(grib_file),istatus)
c         print*,'return status from agrib is',istatus
         if (istatus.ne.0) then
            stop
         endif
      else if(grib_type == 2) then
         ret = agrib2(nx,ny,nz,n_g3_fields,g3_fields,
     *        n_g2_fields,g2_fields,g3,g2,
     *        trim(grib_file),istatus)
c         print*,'return status from agrib2 is',istatus
         if (istatus.ne.0) then
            stop
         endif
      endif
...
c           second call to getacars
      ...
      itimes=1
      istatus1 = -1
      call getacars(grid_type,itimes,adatep,nump,timep,xp,yp,pp,htp,tp,
     *rhp,up,vp,pindexp,
     *latp,lonp,tailnump,a_hdg,a_mach,cal_secs,istatus1,sdg_ap_idp,
     *up_dnp,rh_uncp,tasp,dataSourcep,alat1,elon1,elonv,alattan,dx,
     *nx,ny,nz,rotcon,plev)
...
c     second hour
      itimes=2     
      istatus2=-1
      call getacars(grid_type,itimes,adate, numc,timec,xc,yc,pc,htc,tc,
     *rhc,uc,vc,pindexc,
     *latc,lonc,tailnumc,a_hdg,a_mach,cal_secs,istatus2,sdg_ap_idc,
     *up_dnc,rh_uncc,tasc,dataSourcec,alat1,elon1,elonv,alattan,dx,
     *nx,ny,nz,rotcon,plev)
...
c       write(6,219) i,j,k,plev(k),fht(i,j,k,l),ft(i,j,k,l),frh(i,j,k,l),fu(i,j,k,l), fv(i,j,k,l)
...
c
      ...
      call bilin_maps_sfc(nx,ny,fht(1,1,k,l),
     *ix(n),jy(n),result1)
c
      call bilin_maps_sfc(nx,ny,fht(1,1,k-1,l),
     *ix(n),jy(n),result2)
c
...
c
      call bilin_maps_sfc(nx,ny,ft(1,1,k,l),
     *ix(n),jy(n),result1)


      call bilin_maps_sfc(nx,ny,ft(1,1,k-1,l),
     *ix(n),jy(n),result2)

      write(42,3253) result1,result2, k,p(n)
...
c
      ...
      call bilin_maps_sfc(nx,ny,frh(1,1,k,l),
     *ix(n),jy(n),result1)
c
      call bilin_maps_sfc(nx,ny,frh(1,1,k-1,l),
     *ix(n),jy(n),result2)
c
c     now do the linear interpolation in log-p for result1 and result2
c
...
c
      call bilin_maps_sfc(nx,ny,fu(1,1,k,l),
     *ix(n),jy(n),result1)
c
      call bilin_maps_sfc(nx,ny,fu(1,1,k-1,l),
     *ix(n),jy(n),result2)
c
      if(result1.eq.-99999.9.or.result2.eq.-99999.9) then
         uf(n,l) = -99999.9
...
c
      call bilin_maps_sfc(nx,ny,fv(1,1,k,l),
     *ix(n),jy(n),result1)
c
      call bilin_maps_sfc(nx,ny,fv(1,1,k-1,l),
     *ix(n),jy(n),result2)
c
      if(result1.eq.-99999.9.or.result2.eq.-99999.9) then
           vf(n,l) = -99999.9
...
c     rotate wind fcst into wrt TRUE NORTH, unless it is grid 22 (RRFS)
c     
        if(uf(n,l).ne.-99999.9.and.vf(n,l).ne.-99999.9) then
           if(grid_type .eq. 20 .or. grid_type .eq. 21 .or. grid_type
     *        .eq. 22)  then
c           if(grid_type .eq. 20 .or. grid_type .eq. 21) then
              lat_0_deg = alat1
              lon_0_deg = elon1
     ...
     *                         theta)
              ...
              theta = elonv -rlon(n)
              do while(theta.lt.-180) 
                 theta = theta + 360.
              end do
              do while(theta.gt.180) 
                 theta = theta - 360.
              end do
              theta = rotcon*theta * 0.017453
              uorig = uf(n,l)
              vorig = vf(n,l)
              uf(n,l) = uorig * cos(theta) - vorig * sin(theta)
              vf(n,l) = uorig * sin(theta) + vorig * cos(theta)
c     write(6,3255) p(n),rlat(n),rlon(n),dirf,sfl
 3255         format('p,lat/lon',5f10.2)
           endif
c          call UVTODIRSPD(uf(n,l),vf(n,l),dirf,sfl)
        endif
c
c     now compute stats
c
...
c     compute yesterday's adate
      ...
      nhr = ihr - proj(i)
c      print*, 'proj is ',proj,' nrh is ',nhr,' i ia ',i
      if(nhr.lt.0) then
                   nhr = nhr + 24
...
c
      subroutine getacars(grid_type,itimes,adate,num,time,x,y,p,ht,t,rh,u,v,
     *pi,
     *lat,lon,tn,a_hdg,a_mach,cal_secs,istatus,sdg_ap_id,
     *up_dn,rh_unc,tas,data_source,alat1,elon1,elonv,alattan,dx,
     *nx,ny,nz,rotcon,plev)
c
      include 'netcdf.inc'
      INTEGER NCID, STATUS, grid_type
      real lat_0_deg,lon_0_deg,lat_g_SW_deg,lon_g_SW_deg ! syntactic sugar only
...
c
c     13km stuff for w3fb11
c
c     input args
      real alat1,elon1,elonv,alattan,dx,rotcon
      integer nx,ny,nz

      integer plev(nz)
c      print *,'grid parameters in getacars',alat1,elon1,elonv,alattan,
c     *dx,nx,ny,nz,rotcon

c  if needed for grid_type 20 or 21 (rotated latlon)
      lat_0_deg = alat1
      lon_0_deg = elon1
      lat_g_SW_deg = elonv
      lon_g_SW_deg = alattan
      print*,'grid parameters',lat_0_deg,lon_0_deg,lat_g_SW_deg,lon_g_SW_deg

       call ncpopt(ncverbose)
       print*,'grid_type is',grid_type
      acars_file = 'ACARS_RETRO_DIR' // '/' // adate // 'q.cdf'
      print*,'acars file is',acars_file
...
c
 ...
 480     continue
         pindex = -99
c         print*,'bad pressure for ', tailnum(i),altitude(i),' press: ',pressure
...
c     if errtype is 87 ('W') or 66 ('B')...
         if((errortype(i).eq.87).or.(errortype(i) == 66)) ibad = 1
...
c
         ...
         if(grid_type .eq. 20 .or. grid_typell .eq. 21 .or. grid_type
     *      .eq. 22) then
            call rotll_geo_to_ij(latitude(i),rlon,lat_0_deg,lon_0_deg,
     *                        lat_g_SW_deg, lon_g_SW_deg,dx,
     *                        rjy,rix)
         else
            call w3fb11(latitude(i),rlon,alat1,elon1,dx,
     *        elonv,alattan,rix,rjy)
c         if(i .lt. 100) then
c            write(42,4850) i,latitude(i),longitude(i),rix,rjy
 4850       format('.lat,lon,i,j',i5,1x,4(f8.2,1x))
         endif

c         endif
c restrict to one less than max because bilin interp looks at
c i+1 and j+1
         if(rix.lt.0.or.rix.gt.nx-1) rix = 99.9
         if(rjy.lt.0.or.rjy.gt.ny-1) rjy = 99.9
c     
c     
...
c      close(25)
 ...
 679  format(i6, ' acars in range for date = ',a9)
      istatus=0
      return
      end
c      
...
c**************************************************
c*****************************************************
       SUBROUTINE W3FB11(ALAT,ELON,ALAT1,ELON1,DX,ELONV,ALATTAN,XI,XJ)
...
C
C USAGE:  CALL W3FB11 (ALAT,ELON,ALAT1,ELON1,DX,ELONV,ALATTAN,XI,XJ)
...
C                THE CONE FLAT.
C     ALATTAN   - THE LATITUDE AT WHICH THE LAMBERT CONE IS TANGENT TO
...
C
         ...
         IF (XJ.LT.1.)  XJ = XJ - 1.
C
c         print*,ALAT1,ELON1,DX,ELONV,ALATTAN
c         print*,'out of w3fb11 with alat,elon,xi,xj',alat,elon,xi,xj
...
c       write(6,1001) i,j
 1001   format('.interp SW ',i3,1x,i3)
