...
my $n_args = @ARGV;
if($n_args <4) {
    print "usage: ./gen_retro_acars_stats3.pl <exp name> <startSecs> <endSecs> <number of z levels> {<1= reprocess, 0=no reprocess>}.\n";
    die;
}
my $i_arg=0;
my $model = $ARGV[$i_arg++];
...
if($grid_type == 22) {
	# RRFS NA 13km rotLL grid
	$la1 = 54.0;		# lat_0_deg
	$lo1 = -106.0;		# lon_0_deg
	$lov = -8.052698;	# lat_g_SW_deg
	$latin1 = -139.561;	# lon_g_SW_deg
	$dx = 13.0;
	$grib_nz = 37;
} 
