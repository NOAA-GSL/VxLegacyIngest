C     this routine will verify GFS
...
c still hardwired for gfs parameters, but changed to write an output file rather than
c directly put data into mySQL
    
**************************************************************************
      parameter(maxobs=50000)
c     for GFS 0.5 degree data:
      parameter(nlev=26,nx=720,ny=361,n_g3_fields=5)
      parameter(numbin=26)
      parameter(numproj=1)
      parameter(nreg=3)
...
***************************************************************************
      ...
      print *,'in varif_gfs, valid_secs is ',valid_secs,trim(output_file)
      do i = 1,maxobs
      do j = 1,numproj
      tdif(i,j) = -9.9
      vdif(i,j) = -9.9
