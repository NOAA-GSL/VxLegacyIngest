sub load_summaries_retro2 {
    ...
    my $table_string = "(acars_retro as o use index (date),${model} as m) left join retro_rejects rj on o.xid = rj.xid";
    my $match_string = "and o.aid = m.aid";
    my $fcst_len_string = "and m.fcst_len = $fcst_len";
    if($region eq "HRRR") {
    ...
    my $model_table = "${model}";
my  $query=<<"EOI"
replace into $table (date,hour,fcst_len,up_dn,mb10,N_dt,sum_ob_t,sum_dt,sum2_dt,N_dw,sum_ob_ws,
