...
#    elif (model.startswith(('rap','ruc'))):
    ...
    os.system('mkdir %skm' % radius_s)
    work_dir = radius_s + 'km'
    os.chdir(work_dir)
    ...
    ncl_obs_file = '%s_%s' % (obs_file,model_grid)
    ...
    for forecast_lead in forecast_leads:
        ...
        ncl_model_file = '%s_%s' % (ff_file,model_grid)

        # Write namelist
        nml = open(main_nml_file,'w')
        nml.write("&main_nml" + '\n')
        nml.write("   model_grid = '%s' " % (model_grid) + '\n')
        nml.write("   interp_grid = '%s' " % (model_grid) + '\n')
        ...
        # Copy model file and rename
        #if(os.path.isfile('../%s.nc' % ff_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
        #    cmd = 'cp ../%s.nc %s.nc' % (ff_file,ncl_model_file)
        #    print cmd
        #    os.system(cmd)
        ...
        for threshold in thresholds:
            grid_file = '%s_vs_%s_%sdBZ_%sverif_grids.nc' %  (obs_file,ff_file,threshold,model_grid)
            ncl_verif_file = '%s_vs_%s_%sdBZverif_%s' % (obs_file,ff_file,threshold,model_grid)
            ...
            nml.write("   radius = %s " % (radius_s) + '\n')
            nml.write("   resolution = %s " % (resolution) + '\n')
