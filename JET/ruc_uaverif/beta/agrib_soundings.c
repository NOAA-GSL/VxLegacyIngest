...
{
  int hydro=1;			/* =1 if we are to put out hydrometeor information */
  int result;			/* stores return code for reads */
  raob_data_struct raob_data[MAX_LEVELS];
  float Z,Z1,VPT,VPT1,QV,QV1;
  float U,U1,V,V1;
  float P,P1;
  ...
  int g3_fields[] = {1,7,189,53,33,34,153,170,171,178,179};
  int n_g3_fields = 11;
  ...
  if(argc < 8) {
    printf("Usage: agrib_soundings.x model filename station_file"
  ...
  p_levels = 50;
   for(i=0;i<p_levels;i++) {
     ...
     VPT = g3[startx + nx*(starty + ny*(i + nz*field_index++))];
     QV =  g3[startx + nx*(starty + ny*(i + nz*field_index++))];
     ...
     /*
     printf("press: %.6g, Z: %.6g, VPT: %.2f, QV: %.2g, CLWMR: %.2g, lnCLWMR: %.2g,"
	    " lnRWMR: %.2g, lnSNMR: %.2g, lnICMR: %.2g, lnGRMR: %.2g\n",
	P,Z,VPT,QV,CLWMR,lnCLWMR,lnRWMR,lnSNMR,lnICMR,lnGRMR);
     */
     pr=P/10 + 0.5;		/* convert to millibars times 10 */
     /* printf("press: %.6g, Z: %.6g, VPT: %.2f, QV: %.2g, U: %.2g, V: %.2g\n",
	P,Z,VPT,QV,U,V);*/
     dewpoint(P,VPT,QV,&t_kelvin,&td_kelvin);
     if(pr < last_pressure) {
       last_pressure = pr;
	 ...
	 VPT1 = g3[startx + nx*(starty + ny*(i+1 + nz*field_index++))];
	 QV1 =  g3[startx + nx*(starty + ny*(i+1 + nz*field_index++))];
	 ...
	 pr_next = P1/10 + 0.5;
	 dewpoint(P1,VPT1,QV1,&t_kelvin,&td_kelvin);
