...
/**
 * Change history:
 * 24-Mar-2006 - adapted from sdg.SoundingLoader to read files locally
 */
...
Sounding load_sounding(String directory, String data_source,String site,int wmoid,
  ...
  try {
    ...
    if(data_source.equals("RAOB")) {
      String raob_file_header = System.getenv("raob_file_header");
      // create two temporary files, one for the input
      // and one for the output of the RAOB getter program
      String cwd = new File(".").getCanonicalPath();
      //Debug.println ("cwd is "+cwd);
      raob_input = new File(cwd+"/"+raob_file_header+"_input.tmp");
      raob_input.delete();
      raob_input.createNewFile();
      PrintStream ps = new PrintStream(raob_input);
      // allow 1hr early and late for RAOBs
      // they'll still be stored in the db with the nominal date/hour.
      UTCDate start_date = date.cloneDate();
      start_date.addHours(-1);
      String bdate = ""+
	 date.leadingZero(start_date.getYear())+
	 date.leadingZero(start_date.getMonth())+
	 date.leadingZero(start_date.getDay())+
	 date.leadingZero(start_date.getHour());

      UTCDate end_date = date.cloneDate();
      end_date.addHours(1);
      String edate = ""+
	date.leadingZero(end_date.getYear())+
	date.leadingZero(end_date.getMonth())+
	date.leadingZero(end_date.getDay())+
	date.leadingZero(end_date.getHour());
      //Debug.println("bdate: "+bdate+" , edate: "+edate);
      ps.print(""+bdate+"\n"+edate+"\n0\n0\n0\nN\n1\n"+wmoid+
	       "\nFSL\n0\n0\n0\n1\nN\nN\n\n");
      ps.close();
      raob_output = new File(cwd+"/"+raob_file_header+"_output.tmp");
      raob_output.delete();
      raob_output.createNewFile();
      Process proc = Runtime.getRuntime().exec(cwd+"/get_raob.pl "+raob_file_header);
      proc.waitFor();
      proc.destroy();
      soundingURL = new URL("file://"+raob_output.getCanonicalPath());
    } else {
      String model_sounding_file = System.getenv("model_sounding_file");
      String cwd = new File(".").getCanonicalPath();
      model_sounding = new File(cwd+"/"+model_sounding_file);
      model_sounding.delete();
      model_sounding.createNewFile();
      Process proc =
	Runtime.getRuntime().exec(cwd+"/get_sounding.pl "+data_source+" "+site+" "+
				  directory+"/"+site+" "+
				  date.get1970Secs()+"_"+fcst_len+".gz");
      /*Debug.println(cwd+"/get_sounding.pl "+data_source+" "+site+" "+
		    directory+"/"+site+" "+
		    date.get1970Secs()+"_"+fcst_len+".gz");
      */
      proc.waitFor();
      proc.destroy();
      soundingURL = new URL("file://"+model_sounding.getCanonicalPath());
      //Debug.println("soundingURL is "+soundingURL);
    }
    urlc = soundingURL.openConnection();
    ...
    if(!data_source.equals("RAOB")) {
      ...
      gzin = new GZIPInputStream(fin,4096);
	...
	//Debug.println(i+": "+line);
	  ...
	  if(i==-1) {
		...
		short_type="Dn";
	      ...
	      } // NOTE: THE LOGIC BELOW IS BACKWARDS FROM WHAT IT IS FOR RR AND HRRR
	      else if(maybe.equals("analysis")) {
                short_type="A";
                if(fcst_len != 0) {
		  Debug.println("TROUBLE(1): forecast was requested "+
				"but analysis was read: "+maybe);
		}
	      } else if(maybe.equals("dfi")) {
                short_type="DFI";
                if(fcst_len != -99) {
		  Debug.println("TROUBLE(2): forecast was requested "+
				"but analysis was read: "+maybe);
		}
	      } else {
                short_type="F";
