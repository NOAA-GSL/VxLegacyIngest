...
$ENV{db_machine} = "hopper3";
$ENV{DBI_DSN} = "DBI:mysql:soundings:".$ENV{db_machine};
$ENV{DBI_USER} = "wcron0_user";
$ENV{DBI_PASS} = "cohen_lee";
my $skip_db_load = 0; # 1 to NOT store interpolated soundings in the ruc_ua db.
# soundings are always stored in the soundings db in this version.
...
my $query="";
$query =<<"EOI"
    replace into model_raob_soundings
...
foreach my $valid_time (@valid_times) {
	...
	if($desired_filename) {
		...
		print "ALREADY LOADED in MySQL 5.1: $desired_filename\n";
		next;
	    }
...
;
	...
	if($desired_filename) {
		...
		while(<PULL>) {
			...
			# the command below.
			$sth_load->execute($data_source,$name,$sql_date,$fcst_len_for_db,
		...
		my $command = "/opt/java/jdk1.6.0_04/bin/java Verify dummy_dir $data_source ".
...
sub already_loaded($model,$valid_date,$fcst_len,$special_type,$dby) {
    ...
    my $query=<<"EOI";
	select count(*) from soundings.model_raob_soundings
	where 1=1
	and model = '$model'
	and time = '$valid_date'
