...
int main(int argc, char **argv) {
	...
	fprintf(stderr, "   -ij                     next two elements are desired i and j (integers) (binary output only)\n");
	...
	if (strcmp(argv[i],"-ij") == 0) {
	  col_i = atol(argv[i+1]);
	  i++;
	  col_j = atol(argv[i+1]);
	  i++;
	  if(col_i < 0 || col_j < 0) {
	    printf("BAD: col_i = %d, col_j = %d\n",col_i,col_j);
	    exit(1);
	  }
	  continue;
	}
	...
	if ((mode != INVENTORY && output_type != GRIB) || verbose > 1) {
		...
		if (output_PDS_GDS == 1) {
		  ...
		  /* WRM - change to put out only 1 variable, for the i,j location desired */
	            i = nxny * sizeof(float);
	            if (header == simple) fwrite((void *) &i, sizeof(int), 1, dump_file);
		    if(col_i != -1 && col_j != -1) {
		      /* write out only a single i,j value */
		      idx = col_i + col_j*nx;
		      fwrite((void *) &array[idx], sizeof(float), 1, dump_file);
		    } else {
