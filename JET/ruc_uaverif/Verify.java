...
public static void main(String args[]) {
  ...
  try {
	...
	if(!model.equals("RAOB") &&
	  ...
	  db_url = "jdbc:mysql://"+db_machine+"/ruc_ua_sums2";
	  con_sum =
	    DriverManager.
	    getConnection(db_url,"wcron0_user","cohen_lee");
	  if(!con_sum.isClosed()) {
	    System.out.println("connected to ruc_ua_sums2 on MySQLserver on "+db_machine);
	  } else {
	    System.out.println("PROBLEM connecting to ruc_ua_sums2!");
	  }
	  for(int i_reg=0;i_reg<regions.length;i_reg++) {
	    int region = regions[i_reg];
	    Statement stmt_sum = con_sum.createStatement();
	    String table = model+"_reg"+region;
...
"group by date,hour,mb10,fcst_len\n";
		    ...
		    if(sqlw != null) {
		      System.out.println("SQLWarning is "+ sqlw);
		      while((sqlw = sqlw.getNextWarning()) != null) {
			System.out.println("next SQLWarning is "+ sqlw);
		      }
		    }
		    System.out.println(model+" region: "+region+
				  " fcst_len: "+forecast_len+" "+rs_sum);
	  }
	}
      }
