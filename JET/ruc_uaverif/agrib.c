...
/**int agrib_(int *nz_ptr, int *n_g3_fields,int *g3_fields,
	   ...
	   float *G3, float *G2,int *startx,int *starty,
	   char *filename, int *istatus_ptr ) {
* fortran- and C-callable grib reader, adapted from wgrib and earlier agrib
* by Ara Howard (2000) and Bill Moninger (2005)
*
* input arguments
*
...
*             as they appear in struct ParmTable parm_table_ncep_opn
*             below.
* startx - if -1, agrib returns in g2 and g3 as entire x,y arrays.
*          If startx is an integer, agrib returns in g3 a single column
*          of data for the (i,j) point (startx,starty) for each field
*          and in g2 a single number (the surface value at (startx,starty))
*          for each field.  NOTE: startx and starty ARE ZERO BASED, NOT
*          1-BASED.  So, the lower left corner is (0,0) ala C.  This is true
*          even when this is called from fortran.
* starty - see startx
*
* output arguments:
*
...
int wgrib(int nz, int n3d, int n2d, float *G3, float *G2, int *I3, int *I2, char *filename); 
int load_gds(unsigned char *gds, float *I);
/*END MYCODE*/
...
int agrib_(int *nz_ptr, int *n_g3_fields,int *g3_fields,
    ...
    int print_GDS = 0, print_GDS10 = 0, print_PDS = 1, print_PDS10 = 0;
    char *dump_file_name = "dump", open_parm[3];
    int header = 1, return_code = -1;
    int n_fields_filled = 0, fields_to_fill = 0;
    int n_fields_filled_3d = 0, fields_to_fill_3d = 0;
    int g2_index = 0, g3_index = 0, lev_index = 0;;
    int mb_levels[] = {1000,975,950,925,900,875,850,825,800,
		       775,750,725,700,675,650,625,600,
		       575,550,525,500,475,450,425,400,
		       375,350,325,300,275,250,225,200,
		       175,150,125,100};
    int n_mb_levels = 37;
    int o11, o12;		/* used in picking the right surface field */

/*MYCODE*/
/*
This was what was done for the 'wgrib -d all -GDS10 -s foo.grib' option
technically it is NOT the '-s' option, as this version produces NO output.
This version is designed to be used as a fortran/'C' function; see below
notes about 'C' vs. FORTRAN loading by searching for the phrase MYCODE.
*/
    ...
    fields_to_fill_3d = *n_g3_fields * nz;
    fields_to_fill = *n_g2_fields + fields_to_fill_3d;
    printf("%d fields to fill (%d 3d fields) (%d %d %d)\n",
	   fields_to_fill,fields_to_fill_3d,
	   *n_g2_fields,*n_g3_fields,nz);
...
if (output_type == BINARY) {
  ...
  if(PDS_KPDS6(pds) == 116 || /* "%d-%d mb above gnd" for CAPE and CIN */
     PDS_KPDS6(pds) == 200) { /* "atmospheric column" for PWAT */
    /* special fields HARDWIRED FOR CAPE AND CIN IN 255-0 ABV GND LAYER */
    for(i=0;i<*n_g2_fields;i++) {
      /*printf("trying to match %d\n",PDS_PARAM(pds));*/
      if(PDS_PARAM(pds) == g2_fields[i]) {
	/* octets 11 and 12 */
	o11 = PDS_KPDS7(pds) / 256;
	o12 = PDS_KPDS7(pds) % 256;
	if((PDS_KPDS6(pds) == 116 && o11 == 255 && o12 == 0) ||
	   (PDS_KPDS6(pds) == 200)) {
	  /* this field matches */
	  printf("%d filling 2D field |%s| (%d) %s ",
		 n_fields_filled,k5toa(pds),PDS_PARAM(pds),k5_comments(pds));
	  ...
	  if(*startx == -1) {
	    for(y=0; y<ny; y++) {
	      for(x=0; x<nx; x++) {
		*(G2 +x +y*nx +g2_index*nx*ny) = array[x +y*nx]; /*this is 'C' style loading.*/
	      }
	    }
	  } else {
	    /* a specific i j was input */
	    x = *startx;
	    y = *starty;
	    *(G2 +g2_index) = array[x +y*nx]; /*this is 'C' style loading.*/
	  }
	  n_fields_filled++;
	  break;
	}
  ...
  } else if(PDS_KPDS6(pds) == 109) {
    /* hybrid levels field */
    for(i=0;i<*n_g3_fields;i++) {
      if(PDS_PARAM(pds) == g3_fields[i]) {
	/* this field matches */
	if(PDS_KPDS7(pds) == 1) {
	  printf("filling hybred lev %d field |%s| (%d) %s\n",
		 PDS_KPDS7(pds),k5toa(pds),PDS_PARAM(pds),k5_comments(pds));
	}
	g3_index = i;
	if(PDS_KPDS7(pds) <= nz) {
	  lev_index = PDS_KPDS7(pds)-1; /* index is zero based, levels are 1-based */
	  if(*startx == -1) {
	    for(y=0; y<ny; y++) {
	      for(x=0; x<nx; x++) {
		*( G3+x +y*nx +lev_index*nx*ny +g3_index*nx*ny*nz) = array[x +y*nx]; /*this is 'C' style loading.*/
	      }
	    }
	  } else {
	    /* a specific i j was input */
	    x = *startx;
	    y = *starty;
	    *( G3 +lev_index +g3_index*nz) = array[x +y*nx]; /*this is 'C' style loading.*/
	  }
	  n_fields_filled++;
	  n_fields_filled_3d++;
	}
  ...
  } else if(PDS_KPDS6(pds) == 107) {
    /* sigma field */
    for(i=0;i<*n_g3_fields;i++) {
      if(PDS_PARAM(pds) == g3_fields[i]) {
	/* this field matches */
	printf("%d: filling sigma lev %d field |%s| (%d) %s\n",
	       n_fields_filled,PDS_KPDS7(pds),k5toa(pds),PDS_PARAM(pds),k5_comments(pds));
	g3_index = i;
	if(PDS_KPDS7(pds) <= nz) {
	  lev_index = PDS_KPDS7(pds)-1; /* index is zero based, levels are 1-based */
	  if(*startx == -1) {
  ...
  } else if(PDS_KPDS6(pds) == 100) {
    /* pressure levels field (in mb)*/
    for(i=0;i<*n_g3_fields;i++) {
      if(PDS_PARAM(pds) == g3_fields[i]) {
	/* this field matches */
	g3_index = i;
	/* get index for this pressure */
	for(lev_index=0;lev_index<n_mb_levels;lev_index++) {
	  if(PDS_KPDS7(pds) == mb_levels[lev_index]) {
	    if(lev_index == 0) {
	      printf("filling  %d mb field |%s| (%d) %s as lev index %d\n",
		     PDS_KPDS7(pds),k5toa(pds),PDS_PARAM(pds),k5_comments(pds),lev_index);
	    }
	    /* fill this level */
	    if(*startx == -1) {
	      for(y=0; y<ny; y++) {
		for(x=0; x<nx; x++) {
		  *( G3+x +y*nx +lev_index*nx*ny +g3_index*nx*ny*nz) = array[x +y*nx]; 
		}
	      }
	    } else {
	      /* a specific i j was input */
  ...
  if(n_fields_filled_3d == fields_to_fill_3d) {
    *istatus_ptr=0;
    /* printf("all %d 3d fields filled. Partial SUCCESS\n",fields_to_fill_3d);*/
  }
  if(n_fields_filled == fields_to_fill) {
    printf("all %d fields filled. SUCCESS\n",fields_to_fill);
    *istatus_ptr=0;
    return(1);
  }
...
/* the following block of code realloc's some memory and DOES NOT FREE IT.  CHECK HERE FOR MEMORY PROBLEMS. */
/*MYCODE*/

    /*if(load_gds(gds,I) == -1) exit(1);*/
/*END MYCODE*/

    fclose(input);
    /*  *istatus_ptr=0; turned off by wrm */
    return (return_code);
...
int load_gds(unsigned char *gds, float *I){
   ...
   /*if( (I= (float *) realloc((void *) I, sizeof(float)*j))  ==  NULL){*/
   if( (I= (float *) malloc( sizeof(float)*j))  ==  NULL){
