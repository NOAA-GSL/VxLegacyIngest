 ...
 def __init__(self,**p):
    ...
    if self.lambert:
         ...
         self.count = -1
           ...
           self.first_lat = p['first_lat']
           self.first_lon = p['first_lon']
           self.dx = p['dx']
           self.dy = p['dy']
           self.nx = p['nx']
           self.ny = p['ny']
 ...
 def __str__(self):
    if self.lambert:
      return "%s,%s,%s.%s,%s,%s,%s,%s" % \
        (self.name,self.alat1,self.elon1,self.elonv,self.alattan,self.dx,self.nx,self.ny)
    elif self.cylindrical:
      return "%s,%s,%s.%s,%s,%s" % \
        (self.name,self.first_lat,self.first_lon,self.dx,self.nx,self.ny)
 ...
 def latlon2ij(self,alat,elon):
   ...
   if self.lambert:
    ...
    if xi_out < 0 or xi_out >= self.nx-0.5 or yj_out < 0 or yj_out >= self.ny-0.5:
     ...
     yj_out = (self.first_lat - alat)/self.dy
     if elon < 0:
       elon += 360.0
     xi_out = (elon - self.first_lon)/self.dx
     #print elon,alat,xi_out,yj_out
     if xi_out < 0 or xi_out >= self.nx-0.5 or yj_out < 0 or yj_out >= self.ny-0.5:
        xi_out = nan
        yj_out = nan
