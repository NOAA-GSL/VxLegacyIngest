# module make_3h_subregion_sums.py
import sys
import time

def make_3h_subregion_sums(cursor,model,valid_time,fcst_len,ob_table):
...
""" % (valid_time,cutoff)
   ...
   # make 3h model table
   model_table = "precip_mesonets2.{model}_3h_{fcst_len}_{valid_time}".\
...
create temporary  table {model_table} (
  ...
  `valid_time` int(10) unsigned NOT NULL COMMENT 'Valid time, end of ***3h*** accumulation',
   `precip_3h` int(10) unsigned DEFAULT NULL COMMENT 'forecst pcp in THREE hours in hundredths of an inch',
  UNIQUE KEY `time_id` (`valid_time`,`madis_id`),
  UNIQUE KEY `id_time` (`madis_id`,`valid_time`)
)  
select m1.madis_id,m1.valid_time
,m1.precip+m2.precip+m3.precip as precip_3h
from
       ...
       for thresh in threshs:
        for reg in regions:
          for net in nets:
           ...
           if net != 'All':
              table =  "precip_mesonets2_sums.%s_3h_%s_%s" % (model,reg,net)
           dict = {
              'table' : table,
...
sum(if(        (m.precip_3h > {thresh}) and         (o.bilin_pcp_3h > {thresh}),1,0)) as yy,
sum(if(        (m.precip_3h > {thresh}) and NOT (o.bilin_pcp_3h  > {thresh}),1,0)) as yn,
sum(if(NOT (m.precip_3h > {thresh}) and         (o.bilin_pcp_3h > {thresh}),1,0)) as ny,
sum(if(NOT (m.precip_3h > {thresh}) and NOT (o.bilin_pcp_3h > {thresh}),1,0)) as nn
from
{model_table} as m
,{ob_table} as o
