...
def write_station_values(proj,stations,model,valid_time,fcst_len,PCP,accum=0):
    tmp_file = "tmp/%d.%d.%s.%d.PCP_%sh_data.writing" % \
        (os.getpid(),valid_time,model,fcst_len,accum)
    pid = os.getpid()
    ...
    for s in stations:
        ...
        if math.isnan(xi) or math.isnan(yj):
            ...
            if i_pcp > 0:
                ...
                sum_precip_in += i_pcp
            this_str = "%s,%d,%d,%d\n" % \
                (s.id,valid_time,fcst_len,i_pcp)
            out_str += this_str
