...
if __name__== '__main__':
   ...
   run_times = [run_time1- i*3600 for i in range(0,9,3)]
   #run_times = [  1524556800]
   ...
   for run_time in run_times:
    ...
    stations = []
    try:
       f = open("tmp/pcp_mesonet_stations."+str(int(run_time))+".txt","r")
    except:
       print "tmp/pcp_mesonet_stations."+str(int(run_time))+".txt not found!"
       continue
    ...
    for model in models:
        ...
        if True:
            ...
            for fcst_len in fcst_lens:
                j = Process(target=process_forecast3.go,name=model+"-"+str(fcst_len),
                            args=(ns,stations,fcst_len,reprocess,model,run_time,fcst_len,BAD))
                j.start()
                pid = j.pid
                jobs.append((j,pid))
                nf += 1
    ...
    print "all forecasts/models run at", run_str,"took",int(process_secs),"seconds"
