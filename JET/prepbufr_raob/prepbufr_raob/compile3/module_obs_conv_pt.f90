...
! display the content in a conventional obs list
      ...
      subroutine list_conv_snd(this,filename,output_dir)
         ! display the content in a conventional obs list
         class(obs_conv_pt) :: this
         type(obsbase), pointer :: thisobs
            ...
            if(num <= listnum) then
               ...
               if(obslen >=1) then
                  ...
                  do k=1,numlvl
                     ...
                     ! levels with mising T should be ntype 6, evidently
                     ! this is needed to avoid bad heights in the text display with the SkewT plots.
                     ...
                     irange = int(range)
                     !print *,thisobs%name,k,delta_north,delta_east,ibearing,irange,HHMM,rlat,rlon
                     n_p = n_p+1
