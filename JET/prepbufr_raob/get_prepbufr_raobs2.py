...
def write_sounding(output_sdg_list, wmoid,date,cursor,write_to_db):
    ...
    try:
        ...
        #print("DUPLICATE FOR {}".format(wmoid))
    except(KeyError):
        soundings_count[wmoid] =1
    #print(output_sdg)
...
print("namelist file is {}".format(namelist_file))
try:
    result =subprocess.check_output(['prepbufr2txt.ian.exe'])
...
fout = open("{}RAOBS.{}".format(pb_output_dir,namelist_type),'w')
for wmoid in sorted(soundings_dict.keys()):
    if(soundings_count[wmoid] > 1):
        print("{} duplicate entry for {}. Last one stored in RAOB_raob_soundings".\
              format(soundings_count[wmoid]-1,wmoid))
