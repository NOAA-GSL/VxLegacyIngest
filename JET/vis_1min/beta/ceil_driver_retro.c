...
main (int argc, char *argv[])
{
  ...
  int get_stations_retro(STATION *sta[],MYSQL *conn,time_t valid_secs,
		   int DEBUG);
  int fill_ceil_values(STATION *sta[],int n_stations,
  ...
  if(argc < 13) {
    printf("Usage: ceil_driver.x model valid_time filename grib_type tmp_file fcst_len"
  ...
  if(n_stations > 0) {
    n_zero_ceils = fill_ceil_values(STATIONS,n_stations,filename,grib_type,grid_type,tmp_file,
    ...
    if(n_zero_ceils > 100) {
      ...
      printf(" TOO MANY ZERO CEILINGS FOR %s valid %s\n"
	     "%d stations with zero ceils for %d fcst, valid %s\n",
	     model, asctime(gmtime(&valid_secs)),
	     n_zero_ceils, fcst_len, asctime(gmtime(&valid_secs)));
      fclose(mail);
