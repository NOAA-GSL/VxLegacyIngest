#!/usr/bin/perl
#SBATCH -J vis_1min_dr
#SBATCH --mail-user=verif-amb.gsl@noaa.gov                                                                             #SBATCH --mail-type=FAIL
#SBATCH -A amb-verif
#SBATCH -n 1
#SBATCH -p service 
#SBATCH -t 02:00:00
#SBATCH --mem=1G
#SBATCH -D .
#SBATCH -e /home/amb-verif/vis_1min/tmp/vis_1min_dr.e%j
#SBATCH -o /home/amb-verif/vis_1min/tmp/vis_1min_dr.o%j
#
#
use strict;
...
$ENV{DBI_DSN} = "DBI:mysql:vis_1min:wolphin";
$ENV{DBI_USER} = "wcron0_user";
$ENV{DBI_PASS} = "cohen_lee";
my $dbh = DBI->connect(undef,undef,undef, {RaiseError => 1});

my $tmp_file = "tmp/$$.vis_1min_data.tmp";
my $data_file = "tmp/$$.loaded_1min_data.tmp";
...
my $query =<<"EOQ";
    select fcst_interval_min from vis_1min.model_metadata
...
$query =<<"EOQ";
    select max_fcst_hour from vis_1min.model_metadata
    where model = '$data_source'
EOQ
    ;
...
;
    ...
    my @regions = split(/,/,$result[0]);
    $query =<<"EOI"
select count(*) from vis_1min_sums.${data_source}_$regions[0]
