...
#
# Description: script for producing a text file of METAR data for a user designated station and time period
#
# Input:
...
def METAR_main ( station, start_secs, end_secs ):
    ...
    if int(last_time) < int(start_secs) or int(first_time) > int(end_secs):
        msg = "ERROR: No data exists for that station during the time period desired"
        print(msg)
        exit(3)
    ...
    db.execute("""SELECT * FROM obs WHERE sta_id = %s AND time >= %s and time <= %s""", (station_id,start_secs,end_secs))
    if db.rowcount == 0:
