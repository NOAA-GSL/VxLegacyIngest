...
unless ($source) {
    print "usage: ceil_driver_retro_exp_diag.pl {exp_name} <start_secs> <end_secs> [1 = reprocess]\n";
    exit(1);
}
my $data_source = "${source}_exp_diag";
if($qsubbed == 1) {
...
my $arg_unsafe = "$thisDir/ceil_retro_exp_diag.x ".
...
if($grid_type == 20 || $grid_type==21 || $grid_type==22 || $grid_type==23) {
    ...
    $arg_unsafe = "$thisDir/rotLL_ceil_retro_exp_diag.x $data_source $valid_time ".
