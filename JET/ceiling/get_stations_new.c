...
int get_stations(STATION *sta[], MYSQL *conn, time_t valid_secs,
  ...
  char table_name[100];
  int print_this;
  int sta_id,loc_id,elev;
  float lat,lon;
  time_t obs_time;
  STATION *sptr;

  printf("valid_secs in get_stations is %s",
	 asctime(gmtime(&valid_secs)));

  snprintf(query,500,
	   "select ceil.madis_id,ceil.time,(loc.lat/182)*100,(loc.lon/182)*100\n"
	   "from obs as ceil, madis3.obs as madis, madis3.locations as loc where\n"
	   "ceil.madis_id = madis.sta_id\n"
	   "and ceil.time >= %ld - 1800\n"
	   "and ceil.time < %ld + 1800\n"
	   "and ceil.time = madis.time\n"
	   "and madis.loc_id = loc.id",
