...
print "|$source|, $startSecs\n";

unless ($source) {
    print "usage: ceil_driver_retro_exp_diag2.pl {exp_name} <start_secs> <end_secs> [1 = reprocess]\n";
    exit(1);
}
my $data_source = "${source}_exp_diag2";
if($qsubbed == 1) {
...
($file,$type) =
    &get_iso_file3($run_time,$source,$DEBUG,$fcst_len,
		    $start);
...
my $arg_unsafe = "$thisDir/ceil_retro_exp_diag2.x ".
...
if($grid_type == 20 || $grid_type==21 ) {
    ...
    $arg_unsafe = "$thisDir/rotLL_ceil_retro_exp_diag2.x $data_source $valid_time ".
