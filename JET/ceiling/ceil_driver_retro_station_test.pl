...
require "./jy2mdy.pl";
require "./update_summaries_retro_station_test.pl";
...
$ENV{DBI_DSN} = "DBI:mysql:mysql_local_infile=1:ceiling2:$ENV{DB_NAME}";
$ENV{DBI_USER} = "wcron0_user";
$ENV{DBI_PASS} = "cohen_lee";
my $dbh = DBI->connect(undef,undef,undef, {RaiseError => 1});

# create the needed tables
my $table = "${data_source}_LEGACY_retro";
gen_retro_tables($table,$dbh,$DEBUG);
...
if(!$reprocess &&
   already_processed($table,$valid_time,$fcst_len,$DEBUG)) {
    print "\nALREADY PROCESSED: $table $fcst_len h fcst valid at $valid_str\n";
    next;
} else {
...
unless($file) {
    print "file not found for $table $fcst_len h fcst valid at $valid_str.\n";
    next;
} else {
    print "FILE FOUND $table $fcst_len h fcst valid at $valid_str\n";
}
# get grid details
...
my $arg_unsafe = "$thisDir/ceil_retro.x ".
    "$table $valid_time $file $grib_type $grid_type $tmp_file $data_file $fcst_len ".
...
if($n_stations_loaded > 0 &&
   $n_zero_ceilings < 100) {
    update_summaries_retro($table,$valid_time,$fcst_len,$DEBUG);
} else {
