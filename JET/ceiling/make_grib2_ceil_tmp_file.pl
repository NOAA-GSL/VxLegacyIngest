...
while(<I>) {
	...
	$var = $1;
	$lev = 0;
    } elsif(/(HGT):(cloud ceiling)/) {
	$var = $1;
	$lev = 1;
    } elsif(/(CEIL):(cloud ceiling)/) {
	$var = $1;
	$lev = 1;
   # } elsif(/(CEIL):(cloud base)/) {
#	$var = $1;
#	$lev = 1;
    } elsif(/(HGT):(local level type 215 0)/) {  # no longer needed, with '-set center 7' as an arg to wgrib2
        $var = $1;
        $lev = 1;
    }
...
my @var_list = qw(HGT CEIL);	# changed from "(CEIL HGT)" on 7/24/18 by WRM
foreach my $var (@var_list) {
    foreach my $lev (sort {$a gt $b} keys %{$HoH{$var}}) { # this sort puts 'surface' first
