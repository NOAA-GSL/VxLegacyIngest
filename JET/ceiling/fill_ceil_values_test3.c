...
int fill_ceil_values(char * model,STATION *sta[],int n_stations,
  ...
  if(grib_type == 1) {
    ...
    /* grib2 */
    snprintf(line,500,"./make_grib2_ceil_tmp_file_frac_test_ops.pl %s %s %d",
  ...
  for(i=0;i<n_stations;i++) {
    ...
    field_index = 1;		/* DIST, sfc */
    ceil_msl = g2[xi  + nx*(yj  + ny*field_index)];
    field_index = 0;		/* DIST, cld base */
...
/*      if(strncmp(model,"HRRR_OPS_exp_diag",4) == 0) {
	ceil_agl = ceil_msl*0.32808;
      } else {*/
