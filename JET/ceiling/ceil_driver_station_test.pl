...
#SBATCH --mem=1G
#SBATCH -D .
#SBATCH -e /home/amb-verif/ceiling/tmp/ceil_dr.e%j
#SBATCH -o /home/amb-verif/ceiling/tmp/ceil_dr.o%j
#
#
use strict;
...
require "./jy2mdy.pl";
require "./update_summaries_station_test.pl";
require "./get_grid.pl";
...
unless($result) {
    ...
    $query = "create table ${data_source}_LEGACY like template";
    print "$query;\n";
    $dbh->do($query);
...
#       $query = "create table ${data_source}_Areg$region like ${iso}RR1h_Areg0";
        $query = "create table ${data_source}_LEGACY_$region like template";
        print "$query;\n";
        $dbh->do($query);
    }
}
...
if(!$reprocess &&
   already_processed($data_source,$valid_time,$fcst_len,$DEBUG)) {
    print "\nALREADY LOADED: ${data_source}_LEGACY $fcst_len h fcst valid at $valid_str\n";
    next;
} else {
...
unless($file) {
    print "file not found for ${data_source}_LEGACY $fcst_len h fcst valid at $valid_str.\n";
    next;
} else {
    print "FILE FOUND ${data_source}_LEGACY $fcst_len h fcst valid at $valid_str\n";
}
# get grid details
...
my $arg_unsafe = "$thisDir/ceil_new.x ".
    "${data_source}_LEGACY $valid_time $file $grib_type $grid_type $tmp_file $data_file $fcst_len ".
...
if($grid_type == 22 || $grid_type==23 ) {
    # rotLL grid, DETAILS ARE HARDWIRED
    $arg_unsafe = "$thisDir/rotLL_ceil_test.x $data_source $valid_time ".
        "$file $grib_type $grid_type $tmp_file $data_file $fcst_len $DEBUG";
}
...
;
    ...
    my @regions = split(/,/,$result[0]);
    $query =<<"EOI"
select count(*) from ceiling_sums2.${data_source}_LEGACY_$regions[0]
