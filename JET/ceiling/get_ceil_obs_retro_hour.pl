...
$query =<<"EOI"
replace into obs_retro (madis_id,time,ceil,cvr) values(?,?,?,?)
EOI
    ;
my $sth_load_ceil = $dbh->prepare($query);

$query=<<"EOI"
delete from obs_retro
where madis_id = ?
and time != ?
and time >= ?
and time < ?
...
ENGINE = MEMORY
select sta_id,time,lat,lon,cvr,sky_id
from $db.obs_retro as o,$db.sky_retro as sky where
    sky_id = sky.id    
and o.time >= $valid_secs - 1800
and o.time < $valid_secs + 1800
EOI
    ;
...
(index (sta_id,lat,lon,min_dif))
ENGINE = MEMORY
select sta_id,lat,lon,min(cast(abs(time - $valid_secs) as signed)) as min_dif
from x1
group by sta_id,lat,lon
EOI
    ;
if($DEBUG) {print "$query;\n";}
$dbh->do($query);
...
while($sth->fetch()) {
  ...
  # Steps to avoid Null data
    ...
    if($rows_deleted > 0) {
	print "removing less-well-matched time for station $id ($rows_deleted times)\n";
    }
    my $now = time();
    $sth_load_ceil->execute($id,$time,$ceil_dft,$cvr);
    if($count++ < 40) {
	print "CLOUD COVER $id,$time, id in madis3.sky_retro table: $sky_id, |$cvr| $ceil_dft\n";
    }
}
