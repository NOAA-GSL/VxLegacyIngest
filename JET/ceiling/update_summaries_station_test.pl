sub update_summaries {
    ...
    print "FILLING SUMMARY TABLES FOR ${model}_LEGACY for $fcst_len fcst valid at $valid_string\n\n";

    # get station ids to leave out

    my @bad_ids;
    $query =<<"EOI"
select madis_id
from ceiling2.bad_stations
where 1=1
EOI
;
    #print "$query;\n";
...
foreach my $region (@regions)  {
    my $table = "${model}_LEGACY_$region";
...
from
ceiling2.${model}_LEGACY as m,madis3.metars,ceiling2.obs as o,ceiling2.ruc_metars as rm
where 1 = 1
...
select 1*3600*floor((o.time+1800)/(1*3600)) as time,
 ...
 sum(if(NOT (m.ceil < $thresh) and NOT (o.ceil < $thresh),1,0)) as nn
from
ceiling2.${model}_LEGACY as m,madis3.metars,ceiling2.obs as o
where 1 = 1
and m.madis_id = metars.madis_id
and m.madis_id = o.madis_id
and m.fcst_len = $fcst_len
...
and lon > -10200 and lon < -7000];
     ...
     $query .= qq[                                                                                                                       
        and find_in_set("SFO",reg) > 0];
    } elsif($region eq "E_HRRR") {
     $query .= qq[                                                                                                                       
        and find_in_set("E_HRRR",reg) > 0];
    } elsif($region eq "W_HRRR") {
     $query .= qq[                                                                                                                       
        and find_in_set("W_HRRR",reg) > 0];
    } else {
