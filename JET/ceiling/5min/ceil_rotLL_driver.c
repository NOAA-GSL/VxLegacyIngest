...
main (int argc, char *argv[])
{
  ...
  int fill_rotLL_ceil_values(STATION *sta[],int n_stations,
			      char *filename,int total_min,int grib_type,int grid_type, char *tmp_file,int DEBUG);
  void write_ceils_test (STATION *sta[],int n_stations,
		       MYSQL *conn,
  ...
  conn = do_connect("wolphin","wcron0_user","cohen_lee",
		    "ceiling_15",0,(void *)NULL,0);

  n_stations = get_stations(STATIONS,conn,valid_secs,DEBUG);
  printf("%d stations loaded\n",n_stations);

  if(n_stations > 0) {
    n_zero_ceils = fill_rotLL_ceil_values(STATIONS,n_stations,filename,total_min,
    ...
    if(n_zero_ceils > 100) {
      ...
      fclose(mail);
      (void) snprintf(line,200,"/usr/sbin/sendmail jeffrey.a.hamilton@noaa.gov " 
		      "< %.80s",mailFile);
      printf("cmd: %s\n",line);
      system(line);
