...
#SBATCH --mem=1G
#SBATCH -D .
#SBATCH -e /home/amb-verif/ceiling/5min/tmp/ceil_5min_dr.e%j
#SBATCH -o /home/amb-verif/ceiling/5min/tmp/ceil_5min_dr.o%j
#
#
use strict;
...
$ENV{DBI_DSN} = "DBI:mysql:ceiling_5min:wolphin";
...
my $query =<<"EOQ";
    select fcst_interval_min from ceiling_5min.model_metadata
...
$query =<<"EOQ";
    select max_fcst_hour from ceiling_5min.model_metadata
    where model = '$data_source'
EOQ
    ;
...
$dbh->do("use ceiling_5min");
$query = qq(show tables like "$data_source");
print "in ceiling_5min db: $query\n";
...
;
    ...
    my @regions = split(/,/,$result[0]);
    $query =<<"EOI"
select count(*) from ceiling_5min_sums.${data_source}_$regions[0]
