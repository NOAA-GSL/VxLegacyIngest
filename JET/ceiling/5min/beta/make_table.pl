...
$query =<<"EOI"
select regions_name from ceiling.regions_per_model where 1=1
...
$query =<<"EOI"
select fcst_lens from ceiling.fcst_lens_per_model where 1=1
and model = "$data_source"
EOI
;
my @result = $dbh->selectrow_array($query);
...
$query =<<"EOI"
select thresholds from ceiling.thresholds_per_model where 1=1
and model = "$data_source"
EOI
;
my @result = $dbh->selectrow_array($query);
