...
main (int argc, char *argv[])
{
  ...
  if(argc < 13) {
    printf("Usage: ceil_driver.x model valid_time filename grib_type tmp_file data_file fcst_len fcst_min total_min"
  ...
  conn = do_connect("wolphin","wcron0_user","cohen_lee",
		    "ceiling_5min",0,(void *)NULL,0);

  n_stations = get_stations(STATIONS,conn,valid_secs,DEBUG);
  printf("%d stations loaded\n",n_stations);

  if(n_stations > 0) {
    n_zero_ceils = fill_ceil_values(model,STATIONS,n_stations,filename,total_min,grib_type,grid_type,tmp_file,
				     nx,ny,nz,alat1,elon1,dx,elonv,alattan,DEBUG);
    write_ceils(STATIONS,n_stations,conn,model,valid_secs,fcst_len,fcst_min,total_min,data_file,DEBUG);
    if(n_zero_ceils > 100) {
      (void) snprintf(mailFile,80,"/tmp/mailThis.tmp");
      mail=fopen(mailFile,"w+");  
      fprintf(mail,"To: jeffrey.a.hamilton@noaa.gov\n");
      fprintf(mail,"From: ceiling_5min_processing\n");
      fprintf(mail,"Subject: TOO MANY ZERO CEILINGS FOR %s valid %s\n\n",
	      model, asctime(gmtime(&valid_secs)));
      fprintf(mail,"%d stations with zero ceils for %s %d hr %d min fcst, valid %s\n",
	      n_zero_ceils, model, fcst_len, fcst_min, asctime(gmtime(&valid_secs)));
      printf(" TOO MANY ZERO CEILINGS FOR %s valid %s\n"
	     "%d stations with zero ceils for %d hr %d min fcst, valid %s\n",
	     model, asctime(gmtime(&valid_secs)),
	     n_zero_ceils, fcst_len, fcst_min, asctime(gmtime(&valid_secs)));
      fclose(mail);
