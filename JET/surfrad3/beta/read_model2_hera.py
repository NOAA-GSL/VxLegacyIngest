...
def read_model2(model,run_time,fcst_len_mins):
    ...
    if True:
        ...
        if model == "HRRR" or model == "HRRR_WFIP2":
            ...
            print(diffuse)
            if fcst_len_mins >= 15:
                dswrf15 = grbs2.select(parameterName='Downward short-wave radiation flux',stepRange=avg15)[0]
                print 'DSWRF15',
                print(dswrf15)
                MODEL_DSWRF15 = dswrf15.values
                direct15 = grbs2.select(parameterNumber=200,parameterCategory=4,discipline=0,stepRange=avg15)[0]
                print 'DIRECT15',
                print(direct15)
            ...
            MODEL_DIFFUSE = diffuse.values
            MODEL_DSWRF = dswrf.values
        elif model.startswith("HRRR") and model.find("retro") > -1:
           dswrf = grbs2.select(parameterName='Downward short-wave radiation flux')[0]
           MODEL_DSWRF = dswrf.values
           if False:
              for key in sorted(dswrf.keys()):
               if key == "values":
                 print( 'key is |%s|' % (key,))
                 try:
        ...
        elif model.startswith("HRRR") and model.find("ret") > -1:
           dswrf = grbs2.select(parameterName='Downward short-wave radiation flux')[0]
           MODEL_DSWRF = dswrf.values
           if False:
              for key in sorted(dswrf.keys()):
        ...
        elif model.startswith("RAP_RR"):
           dswrf = grbs2.select(parameterNumber=192,parameterCategory=4,discipline=0)[0]
           MODEL_DSWRF = dswrf.values
        elif model.startswith("RAP") or model.startswith("RR"):
           dswrf = grbs2.select(parameterName='Downward short-wave radiation flux')[0]
           MODEL_DSWRF = dswrf.values
           print MODEL_DSWRF
        else:
            # still need this for WRF_solar???
            dswrf = grbs2.select(parameterName='Downward short-wave radiation flux')[0]
            MODEL_DSWRF = dswrf.values
    if base != None:
        BASE = base.values
