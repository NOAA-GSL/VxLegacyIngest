...
print "$query\n";
$dbh->do($query);
$query = "select min(min_d),max(max_d) from 7day";
my @ary = $dbh->selectrow_array($query);
open(LOG,">>query_times.txt");
print "7day: $time_str $info loaded in $query_time seconds\n";
print LOG "7day: $time_str $info loaded in $query_time seconds\n";
print "\tdata from $ary[0] to $ary[1]\n";
print LOG "\tdata from $ary[0] to $ary[1]\n";

# update bad tails 
update_bad_tails($dbh);
