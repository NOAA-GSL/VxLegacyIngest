...
def main():
    ...
    init_file = '%s_%4d%02d%02d%02dz' % (model,init_t.year,init_t.month,init_t.day,init_t.hour)
    cmd = 'cp %s/%s/%s_24hr_total.nc ./%s_1fcst_24hr_total.nc' % (model_dir,init_dir,init_file,model)
    print cmd
    os.system(cmd)
    ff_files.append('%s_1fcst_24hr_total' % model)
    ...
    cmd = '%s/add_precip_totals %s %d %s %s %s.nc' % (exec_dir,model_nc_var,len(sum_files),sum_files[0],sum_files[1],ff_files[1])
    print cmd
    os.system(cmd)
    ...
    ff_files.append('%s_24fcst_24hr_total' % model)
    cmd = '%s/add_precip_totals %s %d' % (exec_dir,model_nc_var,len(sum_files))
    for i in range(0,24):
        cmd += ''.join(' %s' % sum_files[i])
    cmd += ' %s.nc' % ff_files[3]
    ...
    northwest_file = '%s_northwest_%s' % (statfile,postpend)
    ...
    northwest_fileo = 'se_%s' % (postpend2)
    ...
    bools = [do_conus,do_west,do_east,do_ne,do_se,do_northwest]
    boolstrs = ['do_conus','do_west','do_east','do_ne','do_se','do_northwest']
    sfiles = [c_file,w_file,e_file,ne_file,se_file,northwest_file]
    sfileos = [c_fileo,w_fileo,e_fileo,ne_fileo,se_fileo,northwest_fileo]
    ...
    for (ff_file,forecast_len) in zip(ff_files,forecast_lens):
        if (forecast_len == '01'):
            type = '1hr_totals'
