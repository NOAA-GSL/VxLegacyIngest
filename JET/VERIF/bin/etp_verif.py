...
def main():
    ...
    for forecast_lead in forecast_leads:
        ...
        # Call etp_interp to create interpolated obs and model states
        if(os.path.isfile('../%s.nc' % ff_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
            cmd = '%s/etp_interp %s model' % (exec_dir,main_nml_file)
            print cmd
            os.system(cmd)
        ...
        for threshold in thresholds:
            grid_file = '%s_vs_%s_%skft_%sverif_grids.nc' %  (obs_file,ff_file,threshold,interp_grid)
            ncl_verif_file = '%s_vs_%s_%skft_verif_%s' % (obs_file,ff_file,threshold,interp_grid)
            ...
            if(os.path.isfile('%s.nc' % ncl_model_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
                ...
                if(threshold == thresholds[0]):
                    ...
                    cmd = """ncl 'INFILENAME="%s.nc"' 'MODEL="%s"' 'FIELDNAME="model"' 'OUTFILENAME="%s"' %s/plot_etp.ncl""" % (ncl_model_file,ncl_model,ncl_model_file,ncl_script_dir)
                    print cmd
                    os.system(cmd)
                ...
                cmd = """ncl 'INFILENAME="%s.nc"' 'MODEL="%s"' 'FIELDNAME="verif"' 'OUTFILENAME="%s"' %s/plot_etp.ncl""" % (ncl_verif_file,ncl_model,ncl_verif_file,ncl_script_dir)
                print cmd
                os.system(cmd)
