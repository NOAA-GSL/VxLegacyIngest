...
def main():
    ...
    model_nc_var = os.getenv("VARIABLE")    
    ...
    for forecast_lead in forecast_leads:
        ...
        if (model == 'nam_nest' and interp_grid == '80kmLC'):
            inf = '%s/%s/%s' % (rt_dir,valid_dir,ff_file)
            valt = '%4d%02d%02d%02d' % (t.year,t.month,t.day,t.hour)
            intt = '%4d%02d%02d%02d' % (ff_t.year,ff_t.month,ff_t.day,ff_t.hour)
            # Plot native-grid NAM NEST grids
            # Call NCL program to create model graphic
            cmd = """ncl 'INFILENAME="%s.nc"' 'MODEL="%s"' 'VALIDTIME="%s"' 'INITIALTIME="%s"' 'FORECASTTIME="%s"' 'OUTFILENAME="%s"' %s/plot_NAM_NEST.ncl""" % (inf,ncl_model,valt,intt,forecast_lead,inf,script_dir)
            print cmd
            os.system(cmd)
            os.system("mv %s.000001.png %s.png" % (inf,inf))
            os.system("convert -trim +repage %s.png %s.png" % (inf,inf))
            os.system("convert -bordercolor black -border 15x15 %s.png %s.png" % (inf,inf))
        ...
        if(os.path.isfile('../%s.nc' % ff_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
            cmd = '%s/cref_interp_nsslwrf %s model' % (exec_dir,main_nml_file)
            print cmd
            os.system(cmd)
