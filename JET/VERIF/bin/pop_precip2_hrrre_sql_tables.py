        ...
        for grid in grids:
            for (domain,sql_domain) in zip(domains,sql_domains):
              for member in members:
                file = '%s/%s/%4d%02d%02d-%02dz/%sLC/verif_%s_mem%s_%s_sub24hr_statistics.txt' % \
                       (rt_dir,model,t.year,t.month,t.day,t.hour,grid,grid,member,domain)
                print file
                
                if (not os.path.isfile(file)):
                    continue

                sql_table = '%s_%s_%s_%s' % (sql_model,member,grid,sql_domain)
