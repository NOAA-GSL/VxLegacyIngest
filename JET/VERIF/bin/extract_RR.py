#!/usr/bin/env python
#======================================================================================
# This script will first untar and then move RR files into their respective valid time
...
def main():
    ...
    rt_dir = '/pan1/projects/nrtrr/verif/cref/RR/realtime'

    cref_var = ':var discipline=0 center=59 local_table=1 parmcat=16 parm=196:'
    pcp_var = ':APCP:'
        ...
        for Day in Days:
            ...
            for Hour in Hours:
                ...
                for forecast_lead in forecast_leads:
                    ...
                    # Get model data
                    forecast_file = '%s00%s00' % (rr_ff,forecast_lead)
                    print forecast_file
                    new_file = 'rr_%4d%02d%02d%02dz+%s' % (ff_t.year,ff_t.month,ff_t.day,ff_t.hour,forecast_lead)
                    
                    if (os.path.isfile('%s' % (forecast_file))):
                        # Ungrib RR file into NetCDF, remove grib2 file
                        cmd = 'wgrib2 %s | egrep "(%s|%ssurface:%d-%d hour acc fcst:)" | wgrib2 -i %s -netcdf %s.nc' %  \
                              (forecast_file,cref_var,pcp_var,int(forecast_lead)-1,int(forecast_lead),forecast_file,new_file)
                        print cmd
                        os.system(cmd)
