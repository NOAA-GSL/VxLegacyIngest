...
def main():
    ...
    if (int(Hour) <= 6 or int(Hour) == 18):
        tref = tval - timedelta(hours=12)
        src_file = tref.strftime('%y%j120000%H')
    elif (int(Hour) == 12):
        tref = tval - timedelta(hours=24)
        src_file = tref.strftime('%y%j12000024')
    else:
        print "can't process this valid time"
        return
