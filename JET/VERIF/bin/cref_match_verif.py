...
#    elif (model.startswith(('rap','ruc'))):
    ...
    postpend = 'statistics'
    c_file = '%s_%s_central.txt' % (statfile,postpend)
    se_file = '%s_%s_southeast.txt' % (statfile,postpend)
    s_file = '%s_%s_south.txt' % (statfile,postpend)
    ne_file = '%s_%s_northeast.txt' % (statfile,postpend)
    n_file = '%s_%s_north.txt' % (statfile,postpend)

    postpend2 = 'out_file'
    c_fileo = 'c_%s' % (postpend2)
    s_fileo = 's_%s' % (postpend2)
    se_fileo = 'se_%s' % (postpend2)
    ne_fileo = 'ne_%s' % (postpend2)
    n_fileo = 'n_%s' % (postpend2)
    ...
    bools = [do_c,do_s,do_se,do_ne,do_n]
    boolstrs = ['do_c','do_s','do_se','do_ne','do_n']
    sfiles = [c_file,s_file,se_file,ne_file,n_file]
    sfileos = [c_fileo,s_fileo,se_fileo,ne_fileo,n_fileo]
    ...
    for forecast_lead in forecast_leads:
        ...
        for threshold in thresholds:
            ...
            if(os.path.isfile('%s.nc' % ncl_model_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
                ...
                cmd = '%s/do_verif_match %s' % (exec_dir,nml_file)
                print cmd
                os.system(cmd)
