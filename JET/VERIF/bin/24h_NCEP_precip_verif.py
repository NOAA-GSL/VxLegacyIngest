...
def main():
    ...
    for i in range(1,3):
        # Calculate offset
        delta_t = timedelta(hours=-int(forecast_lens[0])*i)
        init_t  = t + delta_t
        ...
        cmd = '%s/add_hourly 0 9 3 %s/%s/%s %s/%s/%s_12hr_total.nc true' % (exec_dir,model_dir,init_dir,init_file,model_dir,init_dir,init_file)
        print cmd
        os.system(cmd)
    ...
    for i in range(1,9):
        ...
        sum_files.append('%s/%s/%s_00-03_total.nc' % (model_dir,init_dir,init_file))

    # Now, add eight 3hr totals together
    ff_files.append('%s_8fcst_24hr_total' % model)
    cmd = '%s/add_precip_totals %d' % (exec_dir,len(sum_files))
    for i in range(0,8):
        ...
        for threshold in thresholds:
            ...
            if(os.path.isfile('%s.nc' % ncl_model_file) and os.path.isfile('%s/%s/%s.nc' % (obs_dir,valid_dir,ncl_obs_file))):
                ...
                if(threshold == thresholds[0]):
                    ...
                    os.system("convert -depth 8 -quality 90 %s.png %s.png" % (ncl_model_file,ncl_model_file))
                ...
                os.system("convert -depth 8 -quality 90 %s.png %s.png" % (ncl_verif_file,ncl_verif_file))
