...
def main():
    ...
    for forecast_lead in forecast_leads:
        ...
        if (model == 'hrrr'):
            ...
            forecast_dir = '%s/%s%02d%02d%02d/postprd' % (model_dir,ff_t.year,ff_t.month,ff_t.day,ff_t.hour)
            forecast_file = 'RRFS_CONUS.t%02dz.bgrd3d%02d.tm%02d' % (ff_t.hour,int(forecast_lead),ff_t.hour)
        elif (model == 'sar_fv3'):
            forecast_dir = model_dir
            ...
            #forecast_file = 'RRFS_CONUS.t%02dz.bgsfcf%03d.tm00.grib2' % (ff_t.hour,int(forecast_lead))       
            forecast_file = 'RRFS_CONUS.t%02dz.bgsfcf%03d.tm%02d.grib2' % (ff_t.hour,int(forecast_lead),ff_t.hour)       
