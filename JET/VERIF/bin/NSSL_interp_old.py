...
def main():
    ...
    cmd = '%s/stitch_tiles . %s %s.nc' % (exec_dir,obs_nc_var,mosaic_file)
    print cmd
    os.system(cmd)
    os.system('rm tile*.nc')

    cmd = '%s/rename_variable %s/%s/%s.nc' % (exec_dir,nssl_dir,valid_dir,mosaic_file)
    print cmd
    os.system(cmd)    
    ...
    if (os.path.isfile('%s.nc' % mosaic_file)):
        ...
        for (interp_grid,i) in zip(interp_grids,range(0,5)):
            ...
            nml.write("   obs_nc_var = '%s' " % (nc_out_var) + '\n')
