...
def main():
    ...
    grib_search = '%s/%s%s%s-%s*.MRMS_%s_00.50_%s%s%s-%s*.grib2' % (obs_dir,Year,Month,Day,valid_time,nssl_var,Year,Month,Day,valid_time)
    cmd = 'ls %s' % (grib_search)
    print cmd 
    list_of_files = filter(os.path.isfile,glob.glob(grib_search))
    list_of_files = sorted(list_of_files,key = os.path.getmtime)
    #print list_of_files
    grib = list_of_files[0]
    ...
    if (os.path.isfile('%s.nc' % mosaic_file)):
        ...
        for (interp_grid,i) in zip(interp_grids,interp_grid_opts):
            ...
            if (interp_grid == '03kmLC' or interp_grid == '13kmLC' or interp_grid == '03kmLCsar' or interp_grid == '03kmLCrrfs'):
                hgt_mask = nssl_mask
            else:
                hgt_mask = hrrr_mask
