program grid_ccfp
  ...
  print *, 'Reading NSSL grid file: ', trim(inputGrid)

  status(:) = nf90_NoErr
  status(1) = nf90_open(trim(inputGrid),nf90_nowrite,ncid)
  
  ! Get number of Lats/Lons
  status(2) = nf90_inq_dimid(ncid,"Lat",yDimID)
  status(3) = nf90_inquire_dimension(ncid,yDimID,len = ny)
  
  status(4) = nf90_inq_dimid(ncid,"Lon",xDimID)
  status(5) = nf90_inquire_dimension(ncid,xDimID,len = nx)
  print *, nx, ny
  allocate(lats(nx,ny),lons(nx,ny))
  
  ! Get corner Lat/Lon and grid spacing
  status(6) = nf90_get_att(ncid,nf90_global,'Latitude',lats(1,1))
  status(7) = nf90_get_att(ncid,nf90_global,'Longitude',lons(1,1))
  status(7) = nf90_get_att(ncid,nf90_global,'LatGridSpacing',dy)
  status(7) = nf90_get_att(ncid,nf90_global,'LonGridSpacing',dx)
  
  if(any(status(:5) /= nf90_NoErr)) stop "Error reading NSSL grid"
  ...
  lons(1,:) = lons(1,1)
  do i=2,ny
     lats(:,i) = lats(1,i-1) + dy
  enddo

  do i=2,nx
     lons(i,:) = lons(i-1,1) + dx
  enddo
  ...
  ! Loop over 3 forecasts: 4hr, 6hr, 8hr
  do h=4,8,2
     ...
     do while(go_ahead)
        ...
        elseif (tmp(1:4) .eq. 'LINE') then
           ...
           do while(k .le. n)              
              ...
              ! Determine NSSL gridpoint that each vertex lies in
              do i=1,nx
     ...
     ! Define variables
     status(17) = nf90_def_var(ncid,'cov_conf',  nf90_int,  (/xDimID,yDimID/),covVarID)
     ...
     status(22) = nf90_put_var(ncid,covVarID, covs*10 + confs )
