...
! Last Update: April 30 2020 by Jeff Hamilton
!
!=============================================================================================================
use netcdf
...
integer, dimension(200) :: hrrr03_gds, hrrr10_gds, hrrr20_gds, hrrr40_gds, hrrr80_gds, &
                           ...
                           hrrre03_ne_gds, hrrre03_n_gds, sarfv303_gds, &
                           rrfs03_gds
...
logical :: do_conus = .false., do_west = .false., do_east = .false., do_ne = .false., do_se = .false., &
           ...
           do_s = .false., do_n = .false., do_northwest = .false.
...
namelist /verif_nml/                                                                            &
         ...
         do_c, c_out_file, do_s, s_out_file, do_n, n_out_file, do_northwest, northwest_out_file 
...
parameter northwest_domain  = (/ 35.0, 90.0, 0.0, 260.0 /)
...
!:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
! 3KM Lambert Conformal Conic - SAR_FV3 domain
parameter sarfv303_gds = (/ 3,1738,974,int(22.414*1000),int((360.-122.214)*1000),8,int((360.-97.5)*1000), &
                          3000,3000,0,64,int(38.5*1000),int(38.5*1000),0,0,0,0,0,0,255,(0,i=1,180)      /)

! 3KM Lambert Conformal Conic - RRFS_CONUS domain - old!!! 20210429 Hamilton
!parameter rrfs03_gds = (/ 3,1726,974,int(22.414*1000),int((360.-122.214)*1000),8,int((360.-97.5)*1000), &
!                          3000,3000,0,64,int(38.5*1000),int(38.5*1000),0,0,0,0,0,0,255,(0,i=1,180)      /)

! 3KM Lambert Conformal Conic - RRFS_CONUS domain
parameter rrfs03_gds = (/ 3,1746,1014,int(21.886*1000),int((360.-122.174)*1000),8,int((360.-97.5)*1000), &
                          3000,3000,0,64,int(38.5*1000),int(38.5*1000),0,0,0,0,0,0,255,(0,i=1,180)      /)
...
!-------------------------------------------------------------------------------------------------------------
  ...
  subroutine read_cref_grib2_obs_nc_file(obsGrid,var,inputFileName,maskFileName,state)
    ...
    print*, 'size of lats: ', size(lats)
    print*, 'size of lons: ', size(lons)
    print*, 'nx: ', nx
    print*, 'ny: ', ny
    print*, 'size of stat%ats: ', size(state%lat)
  ...
  subroutine read_precip_obs_nc_file(obsGrid,var,inputFileName,maskFileName,state)
    ...
    where(state%precip .gt. 1000) state%missing = .true.
  ...
  subroutine write_interp_state(fieldName,outVar,interpGrid,outFileName,units,state)
  ...
  !  status(1)  = nf90_create(trim(outFileName),nf90_classic_model,ncid)
    ...
    ! Define units 
    status(22) = nf90_put_att(ncid,fieldVarID,'units',units)

    ! End of definitions
    status(23) = nf90_enddef(ncid)
    
    ! Put dimension variables and state variables
    status(24) = nf90_put_var(ncid,fieldVarID,fieldVar)
    status(25) = nf90_put_var(ncid,latVarID,  state%lat)
    status(26) = nf90_put_var(ncid,lonVarID,  state%lon)
    
    if(any(status(:26) /= nf90_NoErr)) stop "Error writing interpolated state to NetCDF file"
  ...
  function get_gds(grid)
    ...
    print *, 'model grid: ', trim(grid)
 
    if(trim(grid) == '03kmLC') get_gds = hrrr03_gds
    if(trim(grid) == '03kmLCsar') get_gds = sarfv303_gds
    if(trim(grid) == '03kmLCrrfs') get_gds = rrfs03_gds
  ...
  function get_domain(domain)
    ...
    if(trim(domain) == 'northwest')   get_domain = northwest_domain

    if(sum(get_domain) == 0) stop 'Domain not recognized'

  end function get_domain
!=============================================================================================================
end module verif_mod2
