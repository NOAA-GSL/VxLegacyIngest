...
if(trim(field) == 'obs') then
   ...
   call read_vip_obs_nc_file(obs_grid,obs_nc_var,obs_in_file,mask_file,obs_state)
   ...
   ! Read model data
   call read_vip_model_nc_file(model_nc_var,model_in_file,model_state)
...
print *, "Program vip_interp finished"
...
!---------------------------------------------------------------------------------
  ...
  subroutine interpolate(switch,old_state,interp_state)
    ...
    if(trim(old_grid) == trim(interp_grid)) then
       ...
       where(old_state%missing) old_state%vip = -9999.
       
       ri(:,km) = reshape(old_state%vip, (/ size(old_state%vip) /) )
       li(:,km) = 0 
       ...
       interp_state%vip = reshape(ro(:,km), (/ xo,yo /) )
       ...
       where(interp_state%vip .eq. -9999.) interp_state%missing = .true. 
       where(interp_state%vip .lt. 0) interp_state%vip = 0
    endif

  end subroutine interpolate

!=================================================================================
end program vip_interp
