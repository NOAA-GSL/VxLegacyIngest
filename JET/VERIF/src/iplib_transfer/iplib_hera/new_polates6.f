...
C     LOCATE INPUT POINTS AND COMPUTE THEIR WEIGHTS
         ...
         IF(IPOPT(2).NE.-1) THEN
            IF(ABS(IB)+ABS(JB) .EQ. 24) THEN
C               WB=REAL(1)/SQRT(REAL(IPOPT(2)))
               WB=REAL(1)/REAL(IPOPT(2))
            ELSEIF(ABS(IB)+ABS(JB) .EQ. 18) THEN
C               WB=REAL(1)/SQRT(REAL(IPOPT(3)))
               WB=REAL(1)/REAL(IPOPT(3))
            ELSEIF(ABS(IB)+ABS(JB) .EQ. 12) THEN
               IF(ABS(IB) .EQ. 0 .OR. ABS(JB) .EQ. 0) THEN
C                  WB=REAL(1)/SQRT(REAL(IPOPT(4)))
                  WB=REAL(1)/REAL(IPOPT(4))
               ELSE
C                  WB=REAL(1)/SQRT(REAL(IPOPT(5)))
                  WB=REAL(1)/REAL(IPOPT(5))
               ENDIF
            ELSEIF(ABS(IB)+ABS(JB) .EQ. 6) THEN
C               WB=REAL(1)/SQRT(REAL(IPOPT(6)))
               WB=REAL(1)/REAL(IPOPT(6))
            ELSE
               WB=REAL(2)
            ENDIF
         ENDIF
         ...
         IF(WB.NE.0) THEN
            DO N=1,NO
               XPTB(N)=XPTS(N)+IB/REAL(NB3)
               YPTB(N)=YPTS(N)+JB/REAL(NB3)
            ENDDO
