...
if(trim(field) == 'obs') then
   ...
   call write_interp_state('obs',nc_out_var,interp_grid,obs_out_file,'dBZ',interp_obs_state)
elseif(trim(field) == 'obs_grib2') then
   ! Read observation data
   call read_cref_grib2_obs_nc_file(obs_grid,obs_nc_var,obs_in_file,mask_file,obs_state)
   ...
   call write_interp_state('model',nc_out_var,interp_grid,model_out_file,'dBZ',interp_model_state)
endif
...
!---------------------------------------------------------------------------------
  ...
  subroutine interpolate(switch,old_state,interp_state)
    ...
    print *, 'old state cref shape: ', shape(old_state%cref)
    print *, 'old state cref size: ', size(old_state%cref)
    print *, 'old state missing shape: ', shape(old_state%missing)
    print *, 'old state missing size: ', size(old_state%missing)
    print *, 'interp state cref shape: ', shape(interp_state%cref)
    print *, 'interp state cref size: ', size(interp_state%cref)
