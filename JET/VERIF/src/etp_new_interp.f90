...
! Echo Top Height.  The input grid can be LamCon or EquiDistant Cyl,
...
if(trim(field) == 'obs') then
   ...
   call read_etp_obs_nc_file(obs_grid,obs_nc_var,obs_in_file,mask_file,obs_state)
...
elseif(trim(field) == 'obs_grib2') then
   ! Read observation data
   call read_etp_grib2_obs_nc_file(obs_grid,obs_nc_var,obs_in_file,mask_file,obs_state)
   ...
   ! Read model data
   call read_etp_model_nc_file(model_nc_var,model_in_file,model_state)
...
print *, "Program etp_interp finished"
...
!---------------------------------------------------------------------------------
  ...
  subroutine interpolate(switch,old_state,interp_state)
    ...
    if(trim(old_grid) == trim(interp_grid)) then
       ...
       where(old_state%missing) old_state%etp = -9999.
       
       ri(:,km) = reshape(old_state%etp, (/ size(old_state%etp) /) )
       li(:,km) = 0 
       ...
       interp_state%etp = reshape(ro(:,km), (/ xo,yo /) )
       ...
       where(interp_state%etp .eq. -9999.) interp_state%missing = .true. 
       where(interp_state%etp .lt. 0) interp_state%etp = 0
