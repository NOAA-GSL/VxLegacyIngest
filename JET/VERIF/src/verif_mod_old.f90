...
integer, dimension(200) :: hrrr03_gds, hrrr10_gds, hrrr20_gds, hrrr40_gds, hrrr80_gds, &
                           nssl01_gds, nssl10_gds, nssl20_gds, nssl40_gds, &
...
logical :: do_conus = .false., do_west = .false., do_east = .false., do_ne = .false., do_se = .false., &
           do_ci = .false., do_hwt = .false., do_roc = .false.
...
parameter nssl01_gds = (/ 0,7000,3500,int(20.005*1000),int((360.-129.995)*1000),8,int((20.005+.01*3499)*1000),  &
                          int((360.-129.995+.01*6999)*1000),int(.01*1000),int(.01*1000),                  &
                          64,0,0,0,0,0,0,0,0,255,(0,i=1,180)                                           /)

! Equidistant Cylindrical (Lat/Lon) ~ 10KM/.1DEG spacing - NSSL domain
parameter nssl10_gds = (/ 0,700,350,int(20.*1000),int((360.-130.)*1000),8,int((20.+.1*349)*1000),      &
                          int((360.-130.+.1*699)*1000),int(.1*1000),int(.1*1000),                      &
                          64,0,0,0,0,0,0,0,0,255,(0,i=1,180)                                           /)

! Equidistant Cylindrical (Lat/Lon) ~ 20KM/.2DEG spacing - NSSL domain
parameter nssl20_gds = (/ 0,350,175,int(20.*1000),int((360.-130.)*1000),8,int((20.+.2*174)*1000),      &
                          int((360.-130.+.2*349)*1000),int(.2*1000),int(.2*1000),                      &
                          64,0,0,0,0,0,0,0,0,255,(0,i=1,180)                                           /)

! Equidistant Cylindrical (Lat/Lon) ~ 40KM/.4DEG spacing - NSSL domain
parameter nssl40_gds = (/ 0,175,88,int(20.*1000),int((360.-130.)*1000),8,int((20.+.4*87)*1000),        &
                          int((360.-130.+.4*174)*1000),int(.4*1000),int(.4*1000),                      &
                          64,0,0,0,0,0,0,0,0,255,(0,i=1,180)                                           /)
...
!-------------------------------------------------------------------------------------------------------------
  ...
  subroutine read_obs_nc_file(var,inputFileName,state)
    ...
    elseif(trim(var) == 'coverage' .or. trim(var) == 'cov_conf' .or. trim(var) == 'prob') then
       allocate(state%cref(nx,ny),state%prob(nx,ny))
       ! For now, assuming that CCFP will be verified against NSSL-CREF
       status(10) = nf90_inq_varid(ncid,'cref',fieldVarID)
       status(11) = nf90_get_var(ncid,fieldVarID,state%cref)
       state%missing = .false.
       state%prob = 0.
       where(state%cref .le. -998) state%missing = .true.
       where(state%cref .ge. 30.) state%prob = 1.
       deallocate(state%cref)
  ...
  subroutine read_model_nc_file(var,inputFileName,state)
    ...
    elseif(trim(var) == 'cov_conf') then
       allocate(state%prob(nx,ny),cov(nx,ny))
       status(12) = nf90_get_var(ncid,fieldVarID,cov)
       state%prob = 0.
       state%missing = .false.
       where(cov .ge. 10) state%prob = .75
       where(cov .ge. 20) state%prob = .40
       where(cov .ge. 30) state%prob = .25
       where(state%prob .le. -998) state%missing = .true.
       if (associated(state%prob)) print *, 'model prob field allocated'
