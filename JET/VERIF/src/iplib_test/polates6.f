...
C      PRINT*,'Calling IJKGDS0'
      CALL IJKGDS0(KGDSI,IJKGDSA)
C      PRINT*,'KGDSI(1): ',KGDSI(1)
C      PRINT*,'IM: ',IJKGDSA(1)
C      PRINT*,'JM: ',IJKGDSA(2)
C      PRINT*,'NSCAN: ',IJKGDSA(6)
C      PRINT*,'KSCAN: ',IJKGDSA(7)
      DO NB=1,NB3
...
CMIC$ DO ALL AUTOSCOPE
          DO K=1,KM
            DO N=1,NO
              IF(N11(N).GT.0) THEN
C              IF(N11(N).GT.0 .AND. GI(N11(N),K).NE.FILL) THEN
