...
C  SPECIAL CASE OF GLOBAL CYLINDRICAL GRID
     ...
     &     KGDSO(11).EQ.0) THEN
          ...
          ENDIF
          IF(ISPEC.EQ.1) THEN
C             CALL SPTRUN(IROMB,MAXWV,IDRTI,IMAXI,JMAXI,IDRTO,IMO,JMO,
C     &                   KM,IPRIME,ISKIPI,JSKIPI,MI,0,0,MO,0,GI,GO)
          ENDIF
C  SPECIAL CASE OF POLAR STEREOGRAPHIC GRID
     ...
     &         KGDSO(8).EQ.KGDSO(9).AND.KGDSO(11).EQ.64) THEN
          ...
          IF(NINT(XP).EQ.IP.AND.NINT(YP).EQ.IP) THEN
            IF(IPROJ.EQ.0) THEN
C              CALL SPTRUNS(IROMB,MAXWV,IDRTI,IMAXI,JMAXI,KM,NPS,
C     &                     IPRIME,ISKIPI,JSKIPI,MI,MO,0,0,0,
C     &                     60.,XMESH,ORIENT,GI,GO,GO2)
            ELSE
C              CALL SPTRUNS(IROMB,MAXWV,IDRTI,IMAXI,JMAXI,KM,NPS,
C     &                     IPRIME,ISKIPI,JSKIPI,MI,MO,0,0,0,
C     &                     60.,XMESH,ORIENT,GI,GO2,GO)
            ENDIF
            ISPEC=1
          ENDIF
C  SPECIAL CASE OF MERCATOR GRID
        ELSEIF(KGDSO(1).EQ.1) THEN
          ...
          IF(NSCANO.EQ.0) THEN
C            CALL SPTRUNM(IROMB,MAXWV,IDRTI,IMAXI,JMAXI,KM,NI,NJ,
C     &                   IPRIME,ISKIPI,JSKIPI,MI,MO,0,0,0,
C     &                   RLAT1,RLON1,DLATO,DLONO,GI,GO)
            ISPEC=1
          ENDIF
        ENDIF
C  GENERAL SLOW CASE
        IF(ISPEC.EQ.0) THEN
C          CALL SPTRUNG(IROMB,MAXWV,IDRTI,IMAXI,JMAXI,KM,NO,
C     &                 IPRIME,ISKIPI,JSKIPI,MI,MO,0,0,0,RLAT,RLON,GI,GO)
        ENDIF
