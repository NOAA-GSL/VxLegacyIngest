...
void write_stations (STATION *sta[],int n_stations,
  ...
  for(i=0;i<n_stations;i++) {
	...
	if(comp_vars(sp->r_land,sp->r_water,
      ...
      /* put the 'best' values into this table */
      snprintf(query,500,"REPLACE INTO %sa "
	...
	exit(1);
      ...
      if(sp->ndiff > 0) {
	snprintf(query,500,"REPLACE INTO %s_coastal "
	...
	if(sp->r_land < 0) {
	  snprintf(query,500,"UPDATE %s_coastal "
	...
	snprintf(query,500,"REPLACE INTO stations_%s_coastal "
