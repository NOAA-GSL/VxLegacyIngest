...
void fill_surface_values_q_all(STATION *sta[],int n_stations,
			   ...
			   int rh_flag,int wind_rot_flag, int DEBUG) {
  FILE *input;
  char line[500];
  STATION *sp;
  ...
  if(grid_type == 20) {
    ...
    printf("3-d fields %d %d %d %d\n",
	   n_g3_fields,nx,ny,nz);

    printf("dx %.2f\n",
	   dx);
    printf("wind rotation flag is %d\n",wind_rot_flag);
 ...
 *          This is a crude way to skip stations affected. - Hamilton 20200429 */
    ...
    */
    /* modify there */
    if (grid_type==3 && wind_rot_flag==0){ 
      /*polar*/
      dlon = elonv - elon;
      while(dlon > 180) {dlon -= 360;}
      while (dlon < -180) {dlon += 360;}
      theta = - dlon;
    } else if (grid_type==1 && wind_rot_flag==0){ 
       /*lambert*/
