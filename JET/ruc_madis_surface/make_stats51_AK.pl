...
#SBATCH --mem=1G
#SBATCH -D .
#SBATCH -o /home/amb-verif/ruc_madis_surface/tmp/madis_hrrr_ak_stats.o%j
#SBATCH -e /home/amb-verif/ruc_madis_surface/tmp/madis_hrrr_ak_stats.e%j
#
use strict;
...
my $n_rows;
my $table;
print "Calculating AK surface stats for $n_days days, ending $back_end_days ago\n\n";
...
$query =<<"EOI"
select valid_day,hour
from surface_sums.HRRR_AK_metar_v2_AK
where 1=1
...
$sth->execute();
# TURN LINE BELOW OFF FOR DEBUGGING
$sth->bind_col( 1, \$startSecs );
while($sth->fetch()) {};
my $time_str = gmtime($startSecs);
print "startSecs is $time_str ($startSecs)\n";
...
from HRRR_AKqp1f as m,stations as s    #, (emacs coloration)
where 1=1
and m.sta_id = s.id
#and fcst_len = 1
and time >= $startSecs
...
# New entries to only grab ASOS/AWOS stations normally inside the METARS net. - Hamilton 20190308

$query =<<"EOQ";
insert into $table
select 'ASOS',n.name,sta_id,time,temp,dp,wd,ws
from HRRR_AKqp1f as m, stations as s, metar_networks as n    #, (emacs coloration)
where 1=1
and m.sta_id = s.id
and m.sta_id = n.madis_id
and n.network = 'ASOS'
#and fcst_len = 1
and time >= $startSecs
and time < $endSecs
...
$query =<<"EOQ";
insert into $table
select 'AWOS',n.name,sta_id,time,temp,dp,wd,ws
from HRRR_AKqp1f as m, stations as s, metar_networks as n    #, (emacs coloration)
where 1=1
and m.sta_id = s.id
and m.sta_id = n.madis_id
...
from obs as o,HRRR_AK_short as m
where 1=1
and m.time = o.time
...
select net,count(distinct m.sta_id) as N_sites
 ...
 from HRRR_AK_short as m,obs_short as o    #, (emacs coloration)
 where 1=1
 and m.sta_id = o.sta_id
 and m.time = o.time
 and o.time >= $startSecs
...
$this_time = time();
$dtime = $this_time - $last_time;
if(0 && system("./make_mesonet_uselist_HRRR_AK.pl")) {
   print"couldn't make mesonet uselist for HRRR_AK: $!\n";
} else {
   print "making HRRR_AK mesonet uselist took $dtime secs\n\n";
}
$last_time = $this_time;
