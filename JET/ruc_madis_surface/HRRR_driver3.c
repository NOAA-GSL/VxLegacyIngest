...
main (int argc, char *argv[])
{
  ...
  if(argc < 13) {
    printf("Usage: ceil_driver.x model valid_time filename fcst_len"
  ...
  if(n_stations > 0) {
    fill_surface_values3(STATIONS,n_stations,filename,
			 nx,ny,nz,alat1,elon1,dx,elonv,alattan,DEBUG);
    write_stations(STATIONS,n_stations,conn,model,valid_secs,fcst_len,DEBUG);
