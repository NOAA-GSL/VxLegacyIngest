...
if ($data_source =~ /AK/) {
   ...
   @regions = qw[AK ALL_RR1 ALL_RUC E_RUC W_RUC ALL_HRRR E_HRRR W_HRRR NORTHWEST];
} elsif ($data_source =~ /^RRFS_NA_3km/) {
   @regions = qw[ALL_HRRR E_HRRR W_HRRR NORTHWEST];
...
@fcst_lens = (1,0,3,6,9,12,15,18,21,24,27,30,33,36,39);
#@fcst_lens = (1);
...
#if($data_source == "RRFSret_nwm_june19_v2_coupled") {
#   $dir = sprintf("RR_retro/$data_source/%4d%2.2d%2.2d%2.2d_2WAY/postprd",
#		    $year+1900,$month+1,$mday,$hour);
#} else {
#$dir = sprintf("RR_retro/$data_source/%4d%2.2d%2.2d%2.2d",
$dir = sprintf("RR_retro/$data_source/%4d%2.2d%2.2d%2.2d/postprd",
		$year+1900,$month+1,$mday,$hour);
#}
...
if($fcst_len == 0) {
    if($data_source =~ /AK/) {
        if($data_source =~ /^RRFS/) {
          $base_file = sprintf("RRFS_CONUS.t%02dz.bgdawp%02d.tm%02d",$hour,$fcst_len,$hour);
        } elsif($data_source =~ /^RR/) {
          $base_file = sprintf("wrfprs_rr_%02d.al00.grb2",$fcst_len);
        } else {
          $base_file = sprintf("wrfprs_hralaska_%02d.grib2",$fcst_len);
        }
    } elsif($data_source =~ /^RRFS_NA_13km/) {
      $base_file = sprintf("RRFS_NA_13km.t%02dz.bgdawp%02d.tm%02d",$hour,$fcst_len,$hour);
    } elsif($data_source =~ /^RRFS_NA_3km/) {
      $base_file = sprintf("RRFS_NA_3km.t%02dz.bgdawpf%03d.tm00.grib2",$hour,$fcst_len);
    } elsif($data_source =~ /^RRFS/) {
      if($data_source =~ /nwm/) {
         $base_file = sprintf("rrfs.t%02dz.prslevf%03d.tm00.grib2",$hour,$fcst_len);
      } else {
         $base_file = sprintf("RRFS_CONUS.t%02dz.bgdawp%02d.tm%02d",$hour,$fcst_len,$hour);
      }
    } elsif($data_source =~ /^HRRR/) {
      $base_file = sprintf("wrfprs_hrconus_%02d.grib2",$fcst_len);
    } elsif($data_source =~ /^RTMA/) {
      $base_file = sprintf("wrfprs_hrconus_%02d.grib2",$fcst_len);
    } else {
      $base_file = sprintf("wrfprs_rr_%02d.al00.grb2",$fcst_len);
    ...
    if($data_source =~ /AK/) {
      ...
      #$base_file = sprintf("RRFS_NA_13km.t%02dz.bgsfcf%03d.tm00.grib2",$hour,$fcst_len);
      $base_file = sprintf("RRFS_NA_13km.t%02dz.bgdawp%02d.tm%02d",$hour,$fcst_len,$hour);
    } elsif($data_source =~ /^RRFS_NA_3km/) {
      $base_file = sprintf("RRFS_NA_3km.t%02dz.bgdawpf%03d.tm00.grib2",$hour,$fcst_len);
    } elsif($data_source =~ /^RRFS/) {
...
# get grid details
print "get_grid.pl $file $thisDir $DEBUG\n";
...
if($grib_type == 2 && $WRF == 1) {
    ...
    my $exit_code = $? >> 8;
    if($exit_code != 0) {
	printf("!!!!! ERROR !!!!! trouble executing $arg\n");
