 ...
 *     lon_g_SW_deg = -55.825378;*/
  ...
  if(grib_type == 1) {
    ...
    printf("nz, %d ,n_g2_fields  %d, no_column %d\n",nz,n_g2_fields,no_column);
    ...
    /* grib2 */
    /* don't deal with vgtyp fim here too*/
    n_g2_fields = 7;
    snprintf(line,500,"./make_grib2_sfc_tmp_file_vgtyp_test.pl %s %s %d %d",
    ...
    floats_to_read_wo_vgtyp = n_g3_fields*nx*ny*nz + n_g2_fields*nx*ny - nx*ny;
    ...
    printf("total floats read: %d, Needed %d\n",floats_read,floats_to_read);
    if(floats_read == floats_to_read) {
      printf("SUCCESS\n");
      printf("%d n_g2_fields. nx=  %d . ny= %d \n",
             n_g2_fields,nx,ny);
    } else if (floats_read == floats_to_read_wo_vgtyp) {
      printf("SUCCESS, but without VGTYP\n");
      printf("%d n_g2_fields. nx=  %d . ny= %d \n",
             n_g2_fields,nx,ny);
    } else {
      printf("%d floats read. Should have read %d or %d without VGTYP\n",
	     floats_read,floats_to_read,floats_to_read_wo_vgtyp);
      exit(8);
    }
...
*/
  ...
  /*test_file = fopen("/whome/amb-verif/ruc_madis_surface/legacy_grid_point_distance_test.txt","w");*/
  ...
  for(i=0;i<n_stations;i++) {
    ...
    delta = sqrt((x_delta*x_delta)+(y_delta*y_delta));
    ...
    for(field_index=0;field_index<6;field_index++) {
      ...
      /* seeing weird values show up randomly in the RRFS dev files.
 *          This is a crude way to skip stations affected. - Hamilton 20200429 */
      if (x00 >= 9999999999.0 || x01 >= 9999999999.0 || x10 >= 9999999999.0 || x11 >= 9999999999.0) {
        /*printf("BAD VALUE! %.1f field_index %d for stat_id: %d, xi: %d yj: %d \n",x00,field_index,sp->sta_id,xi,yj);*/
        /*printf("BAD VALUE! field_index %d for stat_id: %d\n",field_index,sp->sta_id);*/
        n_bad++;
        continue;
      }
      /* end Hamilton */
	...
	(1-rem_x) * (1-rem_y) * x00;
      ...
      if (field_index == 4) {
         if (sp->sta_id == 2105) {
           printf("x00, x01, x10, x11 : %.2f, %.2f, %.2f, %.2f \n",x00,x01,x10,x11);
           printf("temp: %.2f \n",interp_value[field_index]);
	   ...
	   printf("Station id: %d lat: %.2f lon: %.2f x: %.2f y: %.2f\n",
	       sp->sta_id,alat,elon,xreal,yreal);
         }
      }
	 ...
	 (sp->sta_id == 2699 ||	/* KORD */
	  ...
	  sp->sta_id == 223 ||	/* DEN (DIA) */
	  sp->sta_id == 812 ||	/* KPDX (Portland) */
	  sp->sta_id == 2534 ||	/* PVD (Providence) */
	  sp->sta_id == 2105 ||
	  sp->sta_id == 1402 )) {
	printf("\nid: %d, time: %d, lat %.2f, lon %.2f, xi %.2f, yj %.2f, rem_x %.2f, rem_y %.2f\n",
	       sp->sta_id,sp->obs_time,sp->lat,sp->lon,xreal,yreal,rem_x,rem_y);
	printf("index %d, x00: %.2f, x01: %.2f, x10: %.2f, x11: %.2f, interp: %.2f\n",
    ...
    sp->dew_bilin = Td;
    if(rh_flag == 2) {
      /* GFS has no dewpoint; make it a nan */
      sp->dew_bilin = 0.0/0.0;
    }
    sp->rh_bilin = RH;
    sp->vgtyp = (int)VGTYP;
    ...
    */
    ...
    if (grid_type==3){ 
      ...
      while (dlon < -180) {dlon += 360;}
      theta = - rotcon*(dlon);
    } else if(grid_type==11 || grid_type==5) {
      theta = 0;	
    } else if(grid_type == 20 || grid_type == 21) {
      /* rotated LL grid */
    ...
    sp->windDir_bilin = wd;
    /*    if(1) {
      printf("angle Station %d: %.2f %.2f %.2f %.2f\n",
	     sp->sta_id,alat,elon,xreal,yreal);
      printf("angle model U: %.2f, V: %.2f, spd (m/s): %.2f, wd: %d, theta: %.0f\n",
	     U,V,ws,wd,theta);
	     }*/
  ...
  /*fclose(test_file);*/
  printf("%d bad values at stations\n",
	 n_bad/6);
