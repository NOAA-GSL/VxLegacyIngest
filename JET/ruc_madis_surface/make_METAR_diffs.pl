...
$n_days = 30;
$startSecs = 1177113600;
$endSecs = $startSecs + $n_days*24*3600;

my $table_name = "METAR_diff_MPLS_${startSecs}_00_03_6h_${n_days}_day";
$query =<<"EOQ";
...
#explain
select net,count(distinct stations.id) as N_sites,
 from_unixtime(min(o.time)) as min_time,
 from_unixtime(max(o.time)) as max_time,
 count(o.temp) as N_T,
 ...
 from dev as m, dev2 as m2, obs_apr_21_may_21  as o,stations, locations
