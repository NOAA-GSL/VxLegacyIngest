...
int get_stations(STATION *sta[], MYSQL *conn, time_t valid_secs,
 ...
 snprintf(query,500,
	  "select x1.sta_id,x1.loc_id,lat,lon,elev,time,net,slp,temp,dp,ws,wd,name\n"
 ...
 if(DEBUG != 0) {
  ...
  if(mysql_query(conn,query) != 0) {
	...
	while((row = mysql_fetch_row(res_set)) != NULL) {
	  ...
	  sptr->name = strcmp(row[12]);
	  sta[n_stations] = sptr;
	  if(sptr->sta_id == 582) {
            printf("%d %d %.2f %.2f %.0f %ld %.0f %.0f %.0f %.0f %.0f %s\n",
...
void printstation(STATION *s) {
  printf("%s %6d %5.2f %7.2f %4.0f %ld %d ndiff: %d bilin temp, ws: %.2f %.0f",
	 s_>name,s->sta_id,s->lat,s->lon,s->elev,s->obs_time,
	 s->land_wat,s->ndiff,
