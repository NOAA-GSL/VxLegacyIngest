sub get_grid {
    ...
    my($la1,$lo1,$lov,$latin1,$nx,$ny,$dx,$dy,$grib_type,$grid_type,
       $run_date,$fcst_proj);
    $grib_type = 0;
    # first, let's see if its grib(1)
    my $unsafe_arg = qq{$thisDir/col_wgrib.x -V $file};
	...
	if(/date (\d+) .*anl/) {
	   ...
	   #print "DATE IS $run_date $fcst_proj\n";
	    ...
	    $dx = $3;
	    unless($dx > 1000) {
		# its probably in km, so send it out in meters
	...
	if(/Lambert Conformal:/) {
	    ...
	    # on jet. wgrib2 puts out Dx and Dy as zero!  So hack it
	    if($dx == 0 && $nx == 451 && $ny == 337) {
		# its the RUC 13km grid
		$dx = 13545.0;
		$dy = 13545.0;
	    }
	    print "dx |$dx|, dy |$dy|\n";
    ...
    ($sec,$min,$hour,$mday,$mon,$year) = gmtime($valid_secs);
