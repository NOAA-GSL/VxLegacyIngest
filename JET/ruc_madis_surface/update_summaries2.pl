sub update_summaries2 {
    my($model,$valid_time,$fcst_len,$region,$dbh,$DEBUG) = @_;
    use Time::Local;
    use DBI;
    my $query = "";
    my $sth;
    my $table = "surface_sums.${model}_${fcst_len}_metar_$region";
    print "\nupdating summaries in $table\n";
    my $region_string = "";
    if($region eq "E_RUC") {
	$region_string = qq[and lon > -10000];
    } elsif($region eq "W_RUC") {
	$region_string = qq[and lon <= -10000];
    }	
    $query =<<"EOI";
replace into $table
...
from madis3.obs as o,madis3.${model}a as m,madis3.metars as s
where 1=1 $region_string
