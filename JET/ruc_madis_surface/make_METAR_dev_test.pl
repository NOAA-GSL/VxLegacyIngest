...
my $n_days = 1;
my $table_name = "${n_days}_METAR_dev_test";
my ($startSecs,$endSecs);
$endSecs = time()-7200;
$endSecs -= $endSecs % 86400; # start of today
$startSecs = $endSecs - $n_days*24*3600;   # $n_days days back
$query =<<"EOQ";
...
std(sqrt(pow(o.ws,2)+pow(m.ws,2)-2*o.ws*m.ws*cos((o.wd-m.wd)/5729.58)))
 ...
 from dev_test as m, obs as o,stations, locations
 where m.sta_id = o.sta_id
 and m.sta_id = stations.id
 and net = "METAR"
 and o.loc_id = locations.id
 and lat between 38*182 and 49*182
 and lon between -98*182 and -77*182
 and m.fcst_len = 1
 and m.time = o.time
