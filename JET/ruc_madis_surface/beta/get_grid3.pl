...
#           /North pole lat1 ([\-\.\d]+) lon1 ([\-\.\d]+) latD ([\-\.\d]+) LonV ([\-\.\d]+) dx ([\-\.\d]+) dy ([\-\.\d]+)/i;
	    ...
	    print "Rotated Lat-Lon!\n";
	    #since we don't know WHICH rotLL grid we've got, set the grid type to 'unknown'
	    $grid_type = 0;	# unknown
	    $grib_type = 2;
	} else {
	    $grib_type = 2;
	...
	close INVENTORY;
	    print "Xue1 grib_type $grib_type\n";
    ...
    if($grid_type == 0) {
	    ...
	    print "Xue2 grib_type $grib_type\n";
            print "Xue2 corner $corner\n";
        if(defined $corner) {
