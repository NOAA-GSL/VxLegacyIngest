...
main (int argc, char *argv[])
{
  ...
  void fill_surface_values_q_all(STATION *sta[],int n_stations,
				 char *filename,char *tmp_file, int grib_type,int grid_type,int nx, int ny,int nz,
  ...
  printf("%d argc loaded for %d network(s)\n",argc,arg_i);
  arg_i=1;
  nets = argv[arg_i++];
  model = argv[arg_i++];
  ...
  printf("coastal_station_file: %s\n",coastal_station_file);
  (void)sscanf(argv[arg_i++],"%i",&rh_flag);
  printf("rh flag is %d\n",rh_flag);

  printf("%d argc loaded for %d network(s)\n",argc,arg_i);
 
  /* comments out this if(argc >= arg_i) { */
    if(argc > arg_i) { 
  ...
  conn = do_connect("wolphin.fsl.noaa.gov","sfc_driver5","driver5",
  ...
  if(n_stations > 0) {
    ...
    if(strncmp(model,"GLMP",4) == 0) {
      ...
      fill_surface_values_q_all(STATIONS,n_stations,filename,tmp_file,grib_type,grid_type,
			    nx,ny,nz,alat1,elon1,dx,elonv,alattan,rh_flag,DEBUG);
    }
