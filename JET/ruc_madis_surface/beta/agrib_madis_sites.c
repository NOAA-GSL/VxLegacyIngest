...
main (int argc, char *argv[])
{
  ...
  void write_stations5 (STATION *sta[],int n_stations,
			MYSQL *conn,
  ...
  if(argc < 13) {
    printf("Usage: agrib_soundings.x model grib_type valid_time filename fcst_len"
  ...
  if(n_stations > 0) {
    fill_ruc_values(model,grib_type,STATIONS,n_stations,filename,
		    nx,ny,nz,alat1,elon1,dx,elonv,alattan,DEBUG);
    write_stations5(STATIONS,n_stations,conn,model,valid_secs,fcst_len,
