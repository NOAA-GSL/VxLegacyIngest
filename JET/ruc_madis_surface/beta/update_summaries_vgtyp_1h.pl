...
STRAIGHT_JOIN $db_name.${model}qp1f as m #ignore index (time_id)
STRAIGHT_JOIN $db_name.metars as s STRAIGHT_JOIN $db_name.station_vgtyp_$model as v
where 1=1
and o.sta_id = m.sta_id
and o.sta_id = s.madis_id
and o.sta_id = v.sta_id
#and m.fcst_len = $fcst_len
and o.time = m.time
and o.time >= $valid_time - 1800
and o.time < $valid_time + 1800
group by valid_day,hour,vgtyp
EOI
    ; #
if($DEBUG) {
    #print "$query";
