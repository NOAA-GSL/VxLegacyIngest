...
select name,net,min_time ,max_time,
   if(N_T > 84 and bias_T < $bias_T_limit  and std_T < $std_T_limit, 1,0) as T_flag,
   N_T,(avg_T-32)/1.8,bias_T/1.8,std_T/1.8,
   if(N_S > 84 and bias_S < $bias_S_limit and std_S < $std_S_limit and bias_DIR < $bias_DIR_limit
   ...
   rms_W*0.447,
   if(N_Td > 84 and bias_Td < $bias_Td_limit and std_Td < $std_Td_limit,1,0) as Td_flag,
