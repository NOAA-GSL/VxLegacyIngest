...
my $base_file;
if($data_source !~ /^HRRR/ && $fcst_len == 0) {
    $base_file = sprintf("wrfprs_hrconus_%02d.al00",$fcst_len);
} else {
...
unless(-r $file) {
    ...
    print "FILE $file FOUND $data_source $fcst_len h fcst valid at $valid_str\n";
}
# get grid details
...
if($WRF == 1) {
#    if($grib_type ==2) {
#        print ("make_grib2_sfc_tmp_file.pl $file $tmp_file $grib_type $rh_flag\n");
#        my $make_grib = `${thisDir}/make_grib2_sfc_tmp_file.pl $file $tmp_file $grib_type $rh_flag`;
#        print ("$make_grib\n");
#    }
    $arg_unsafe = "${thisDir}/WRF_retro.x ".
	"$data_source $valid_time ${thisDir}/$file $grib_type $grid_type $fcst_len ".
	"$la1 $lo1 $lov $latin1 $dx ".
	"$nx $ny 1 $max_retro_secs ".
	"${thisDir}/$tmp_file ${thisDir}/$data_file ${thisDir}/$data_1f_file ${thisDir}/$coastal_file ${thisDir}/$coastal_station_file ".
	"$rh_flag  $DEBUG";
...
if($n_stations_loaded > 0) {
	...
	update_summaries_retro5($data_source,$valid_time,$fcst_len,$region,
        ...
        print "update_summaries_retro5.pl $data_source $valid_time $fcst_len $region $max_retro_secs $dbh $db_name 0\n";
    }
} else {
