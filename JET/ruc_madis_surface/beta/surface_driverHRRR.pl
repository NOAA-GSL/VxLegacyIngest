...
if($n_stations_loaded > 0) {
    foreach my $region (qw[ALL_HRRR E_HRRR W_HRRR]) {
	update_summaries3($data_source,$valid_time,$fcst_len,$region,$dbh,$DEBUG);
    }
} else {
...
sub already_processed($data_source,$valid_time,$fcst_len,$DEBUG) {
    ...
    my $query =<<"EOI"
	select count(*) from surface_sums.${data_source}_${fcst_len}_metar_ALL_${data_source}
