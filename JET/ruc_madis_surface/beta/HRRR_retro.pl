...
$query = qq(show tables like "${data_source}_0_metar_ALL_HRRR");
my $result = $dbh->selectrow_array($query);
unless($result) {
...
if($WRF == 1) {
	...
	"$nx $ny 1 $endSecs $tmp_file $data_file $data_1f_file $coastal_file $coastal_station_file ".
	"$DEBUG";
...
if($n_stations_loaded > 0) {
	...
	update_summaries_retro5($data_source,$valid_time,$fcst_len,$region,$endSecs,$dbh,$db_name,0);
    }
} else {
