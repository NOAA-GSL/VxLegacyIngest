...
*/
    ...
    fields_to_fill = *n_g2_fields + *n_g3_fields * nz;
    printf("%d fields to fill\n",fields_to_fill);
...
if (output_type == BINARY) {
  ...
  if(PDS_KPDS6(pds) == 109) {
    ...
    /* 2d field, presumably */
    for(i=0;i<*n_g2_fields;i++) {
      /*       printf("field count for %d is %d\n", i,processed_2d_field[i]);*/
      if(processed_2d_field[i] == 0 && /* don't process the same field twice */
	 PDS_PARAM(pds) == g2_fields[i] &&
	/* For 2D fields, only allow these values of KPDS */
	 ((g2_fields[i] == 225 && PDS_KPDS6(pds) == 1) || /* for VGTYP @ 'sfc' */
	  PDS_KPDS6(pds) == 102 ||   /* "MSL" */
	  (PDS_KPDS6(pds) == 105 && /* xx m above ground" */
	   (PDS_KPDS7(pds) == 2 || /* 2 m above ground */
	    PDS_KPDS7(pds) == 10)))) { /* 10 m above ground */
	/* this field matches */
        printf("field count for %d is %d\n", i,processed_2d_field[i]);
        printf("Xue nx, ny for %d is %d\n", nx,ny);
	printf("%d: filling surface field |%s| (%d) %s ",
	       i,k5toa(pds),PDS_PARAM(pds),k5_comments(pds));
	/* print level */
	levels(PDS_KPDS6(pds),PDS_KPDS7(pds));
	printf("\n");
	g2_index = i;
	processed_2d_field[i] = 1;
	if(*startx == -1) {
	  for(y=0; y<ny; y++) {
	    for(x=0; x<nx; x++) {
