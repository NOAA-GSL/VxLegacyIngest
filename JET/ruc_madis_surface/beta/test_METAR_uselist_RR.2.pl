...
$query =<<"EOQ";
select sta_id,m.name,net,min_time ,max_time,
 ...
 from $stats_table as m, metars where 1=1
 and metars.madis_id = m.sta_id
 and net = 'METAR'
 and lat <= 5000 # lat and lon in degrees times 100
 and lat >= 3000
 and lon >= -10300
 and lon <= -8500
EOQ
;
# , (for emacs coloration)
print "$query\n";
...
EOI
    ...
    $query = "drop table if exists uselist";
    print "$query\n";
    $dbh->do($query);
    $query = "alter table uselist_tmp rename as uselist";
    print "$query\n";
    $dbh->do($query);
    unlink($out_db_file) ||
...
my $base_file = "${max_day_found}_METAR_uselist.txt";
...
print OUT "; long -103 to -85 (inclusive)\n";
print OUT ";lat 30 to 50 (inclusive)\n";
