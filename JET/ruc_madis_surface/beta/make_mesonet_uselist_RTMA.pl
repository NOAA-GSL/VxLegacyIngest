...
my $query = "";
my $sth;
my $stats_table = "HRRR_7day";
my $min_N = 20;
my $limits_table = "limits_hrrr";

# get limits from the database
# the limits table is in mks/Celsius units
# but the 7-day stats table is in English units
# FOR RTMA, DOUBLE THE LIMITS (for now)
# except for S_for_DIR_limit
$query = qq|select 
    2*bias_T_limit,2*std_T_limit,2*bias_S_limit,
    2*std_S_limit,2*bias_DIR_limit,2*std_DIR_limit,S_for_DIR_limit,
    2*rms_W_limit,2*bias_Td_limit,2*std_Td_limit
    from $limits_table|;
print "$query;\n";
...
close OUT_DB;
my $tmp_table = "RTMA_uselist_tmp";
$query = "drop table if exists $tmp_table";
print "$query\n";
$dbh->do($query);
$query = "create  table $tmp_table like uselist";
print "$query\n";
$dbh->do($query);
$query =<<"EOI"
...
print "$rows rows loaded into $tmp_table\n";
$query = "drop table if exists RTMA_uselist";
print "$query\n";
$dbh->do($query);
$query = "alter table $tmp_table rename as RTMA_uselist";
print "$query\n";
$dbh->do($query);
unlink($out_db_file) ||
...
my $base_file = "${max_day_found}_RTMA_meso_uselist.txt";
my $uselist_dir = "rtma_mesonet_uselists";
...
print OUT qq{; First, we go through  table \'$stats_table\' and\n}.
    ...
    qq{; wind, temperature, and dewpoint separately (TWICE as specified in table madis3.$limits_table)\n};
...
print OUT "; taken at all hours of the day\n";
