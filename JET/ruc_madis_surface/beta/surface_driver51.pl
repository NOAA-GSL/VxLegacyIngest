...
my $db_machine = $ARGV[1] || "hopper3";
my $db_name = $ARGV[2] || "madis";
$ENV{db_machine} = $db_machine;
$ENV{DBI_DSN} = "DBI:mysql:$db_name:".$ENV{db_machine};
...
my @fcst_lens;
my $WRF;
#foreach my $data_source (qw(RR1h HRRR Bak13)) {
foreach my $data_source (qw(Bak13)) {
    if($data_source =~ /^HRRR/) {
...
if($grib_type == 1 && $WRF == 1) {
    ...
    my $dx_km = $dx/1000;
    my $arg_unsafe = "${thisDir}/agrib_madis_sites.x $db_machine $db_name ".
	"$data_source $valid_time $file $fcst_len ".
