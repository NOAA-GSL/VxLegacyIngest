...
void fill_surface_values_q_polar(STATION *sta[],int n_stations,
 ...
 void rotLL_geo_to_ij(float lat_g_deg,float lon_g_deg,float lat_0_deg,float lon_0_deg,
  ...
  void w3fb06(float alat,float elon,float ALAT1,float ALON1,float DX,float ALONV,float *pxi,float *pxj);
  ...
  for(i=0;i<n_stations;i++) {
    ...
    alat = sp->lat;
    elon = sp->lon;
    /*    printf("Xue Station %d: %.2f %.2f \n", sp->sta_id,alat,elon);*/
    /* xue get the appropriate i,j */
    /*    printf("Xue1 before call w3fb06: alat: %.2f, elon: %.2f \n",alat,elon);*/
    w3fb06(alat,elon,alat1,elon1,dx,elonv,&xreal,&yreal);
    /*    printf("Xue before call w3fb06: xi: %.2f, yi: %.2f \n",xreal,yreal);*/
 ...
 void rotLL_geo_to_ij(float lat_g_deg,float lon_g_deg,float lat_0_deg,float lon_0_deg,
  ...
  for(i=0;i<n_stations;i++) {
    ...
    /* this means that S winds near the E coast need to pick up
       an W component wrt true N.  So rotate the wind vectors
       clockwise (+) near the E coast.
       This code inferred from http://12characters.net/wrfbrowser/html_code/share/wrf_fddaobs_in.F.html
       And was checked by comparing to the wind rotation field for the 242 grid
    */
    /*dlon = elonv - elon;
