...
void fill_surface_values_q_all(STATION *sta[],int n_stations,
  ...
  if(grib_type == 1) {
    ...
    n_g2_fields = 7;
    snprintf(line,500,"./make_grib2_sfc_tmp_file_vgtyp_smartinit.pl %s %s %d %d",
  ...
  for(i=0;i<n_stations;i++) {
    ...
    if (sp->sta_id == 1582) {
      ...
      printf("temp %.3f for x: %d y: %d \n",VGTYP,x_near+1,y_near+1); 
    }
    field_index = 6;
    VGTYP = g2[x_near + nx*(y_near + ny*field_index)];
    if(i < 0) {
    ...
    for(field_index=0;field_index<6;field_index++) {
      ...
      /* seeing weird values show up randomly in the smartinit files.
         This is a crude way to skip stations affected. - Hamilton 20200424 */
	...
	(1-rem_x) * (1-rem_y) * x00;
      ...
      if (field_index == 4) {
	   ...
	   printf("index %d, x00: %.2f, x01: %.2f, x10: %.2f, x11: %.2f, temp K: %.2f\n",
	       field_index,x00,x01,x10,x11,interp_value[field_index]);
	   printf("index %d, x00: %.2f, x01: %.2f, x10: %.2f, x11: %.2f, temp interp: %.2f\n",
  ...
  printf("%d stations with bad values\n",
	 n_bad);
