...
for my $wei_num (0 .. 24)
{
    $valid_times[$wei_num]= $time- $wei_num*3600;
    $valid_str = gmtime($valid_times[$wei_num]);
    print "Xue valid_time:$valid_str\n";
}
...
if($n_stations_loaded > 0) {
	...
	update_summaries_q($data_source,$valid_time,$fcst_len,$region,$dbh,$db_name,1);
    }
