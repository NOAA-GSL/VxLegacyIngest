...
# 1) MODE_compute.py              %%%%%%%%%%%%%%%%%%%%
# 2) aggregate_MODE_scores_TOD.py %%% THIS SCRIPT %%%%
# 3) plot_MODE_results_TOD.py     %%%%%%%%%%%%%%%%%%%%
...
# NOTE: MUST RUN MODE_compute.py first (there is no MODE_compute_TOD.py script...it is not needed).
...
#############################################################################################
# The difference between this script at the similar aggregate_MODE_scores.py script is that #
# this one sorts out verification stats by valid time of day rather than forecast hour.     #
#############################################################################################
...
img_dir = "{}/{}/{}/images/valid_time".format(args.dump_root,name,field)
...
for R in range(1,n_rad+1):
   ...
   for T in range(T_start,n_thresh+1):
      ...
      all_f_p95 = np.empty(0,dtype=np.float)
      ...
      all_o_p95 = np.empty(0,dtype=np.float)
      ...
      for tod in range(tod_start,24):
         ...
         f_p95 = np.empty(0,dtype=np.float)
         ...
         o_p95 = np.empty(0,dtype=np.float)
         ...
         for n in range(1,int(N_fhr_check)+1):
            ...
            while time <= end_time:
               ...
               f_p95 = np.append(f_p95,data_attr['file_f_p95'])
               ...
               o_p95 = np.append(o_p95,data_attr['file_o_p95'])
         ...
         all_f_p95 = np.append(all_f_p95,f_p95)
         ...
         all_o_p95 = np.append(all_o_p95,o_p95)
         ...
         if num_fcst_objs[ff] > 0 and num_obs_objs[ff] > 0:
            ...
            p95_crps = calc_CRPS(f_p95,o_p95,pXX_bins)
            ...
            if True:
               ...
               plt.figure(figsize=(5,5))
               plt.subplots_adjust(left=0.125,bottom=0.1,top=0.98,right=0.98)
               if True in np.isnan(f_p95):
                print("Yeah, there are NaNs in f_p95")
               f_hist,bin=np.histogram(f_p95,bins=np.arange(25.0,70.1,5.0))
               o_hist,bin=np.histogram(o_p95,bins=np.arange(25.0,70.1,5.0))
               plt.bar(0.5*(bin[:-1]+bin[1:]),f_hist/float(num_fcst_objs[ff]),width=bin[1:]-bin[:-1],align='center',color='red',edgecolor='white',linewidth=1,label=name)
               plt.bar(0.5*(bin[:-1]+bin[1:]),o_hist/float(num_obs_objs[ff]),width=1.0*(bin[1:]-bin[:-1]),align='center',color='None',edgecolor='black',linewidth=1,label="MRMS")
               plt.grid(linestyle=":",color='0.75')
               ...
               std_f = np.std(f_p95)
               std_o = np.std(o_p95)
               med_f = np.median(f_p95)
               med_o = np.median(o_p95)
               ...
               plt.figtext(0.14,0.82,r"N$_f$={};  N$_o$={}".format(num_fcst_objs[ff],num_obs_objs[ff]),ha='left',va='top',fontsize=7,bbox=dict(facecolor='white',edgecolor='black',linewidth=0.5))
               plt.figtext(0.14,0.77,"CRPS={:5.3f}".format(p95_crps),ha='left',va='top',fontsize=9,fontweight='bold',bbox=dict(facecolor='white',edgecolor='black',linewidth=1))
               image_file = "{}/{}_object_p95_hist_r{}t{}_{}Z.png".format(img_dir,name,R,T,time_of_day)
               plt.savefig(image_file,dpi=120)
               plt.close()
         ...
         np.savez("{}/mode_metrics_r{}t{}_{}Z".format(dump_dir,R,T,time_of_day),OTS=OTS[ff],MMI=MMI[ff],POD=total_pod[ff],FAR=total_far[ff],SR=total_sr[ff],CSI=total_csi[ff],
                  ...
                  complex_crps=complex_crps,p95_crps=p95_crps,lon_crps=cent_lon_crps,lat_crps=cent_lat_crps,ncases=n_cases,
      ...
      for d in range(0,len(agg_vhr_start)):
         ...
         np.savez("{}/agg_scores_r{}t{}_{:02d}Z-{:02d}Z".format(dump_dir,R,T,agg_vhr_start[d],agg_vhr_end[d]),
     ...
     #  plt.pcolormesh(x2d[:-1,:-1],y2d[:-1,:-1],fbias_hist2d,norm=norm,cmap=final_colors)
      ...
      if True:
         ...
         plt.figure(figsize=(5,5))
         plt.subplots_adjust(left=0.125,bottom=0.1,top=0.98,right=0.98)
         f_hist,bin=np.histogram(all_f_p95,bins=pXX_bins)
         o_hist,bin=np.histogram(all_o_p95,bins=pXX_bins)
         plt.bar(0.5*(bin[:-1]+bin[1:]),f_hist/float(total_fcst_objs),width=bin[1:]-bin[:-1],align='center',color='red',edgecolor='white',linewidth=1,label=name)
         plt.bar(0.5*(bin[:-1]+bin[1:]),o_hist/float(total_obs_objs),width=1.0*(bin[1:]-bin[:-1]),align='center',color='None',edgecolor='black',linewidth=1,label="MRMS")
         plt.yscale('log')
         plt.grid(linestyle=":",color='0.75')
         ...
         plt.tick_params(axis='both',labelsize=6)
         plt.legend(loc=1,prop={'size':8})
         mu_f = np.mean(all_f_p95)
         mu_o = np.mean(all_o_p95)
         std_f = np.std(all_f_p95)
         std_o = np.std(all_o_p95)
         med_f = np.median(all_f_p95)
         med_o = np.median(all_o_p95)
         ...
         plt.figtext(0.97,0.68,r"N$_f$={};  N$_o$={}".format(total_fcst_objs,total_obs_objs),ha='right',va='top',fontsize=7,bbox=dict(facecolor='white',edgecolor='black',linewidth=0.5))
         plt.figtext(0.97,0.63,"CRPS={:5.3f}".format(p95_crps),ha='right',va='top',fontsize=9,fontweight='bold',bbox=dict(facecolor='white',edgecolor='black',linewidth=1))
         image_file = "{}/{}_object_p95_hist_r{}t{}_day.png".format(img_dir,name,R,T)
         plt.savefig(image_file,dpi=120)
         plt.close()
