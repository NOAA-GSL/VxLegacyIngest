...
model_list_ens = os.getenv("MODEL_LIST_ENS")
model_list_ens = model_list_ens.split(" ")
model_list_ens = "</val>\n      <val>".join(model_list_ens)
...
cfg_file_ens = os.getenv("CFG_XML_ENS")
cfg_xml = cfg_dir + "/" + cfg_file
cfg_xml_ens = cfg_dir + "/" + cfg_file_ens
case_cfg_xml = cfg_dir + "/case_mode_" + cfg_file
case_cfg_xml_ens = cfg_dir + "/case_mode_" + cfg_file_ens
...
os.system("cp " + cfg_xml_ens + " " + case_cfg_xml_ens)
fin = open(case_cfg_xml_ens, "rt")
fdata = fin.read()
fdata = fdata.replace("{main_dir}", main_dir)
fdata = fdata.replace("{cycle}", cycle)
fdata = fdata.replace("{model_list}", model_list_ens)
fdata = fdata.replace("{data_types}", data_types)
fin.close()
fin = open(case_cfg_xml_ens, "wt")
fin.write(fdata)
fin.close()

os.system(python_path + " " + mvload_path + " " + case_cfg_xml)
os.system(python_path + " " + mvload_path + " " + case_cfg_xml_ens)
