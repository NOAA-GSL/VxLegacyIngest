...
my $endSecs=0;
my @valid_times;
my $model_name_for_metadata;
my $hours_ago = abs($ARGV[$i_arg++]) || 0;
# see if this is a retro run
if($hours_ago > 631152000) {	# Mon 1 Jan 1990 00:00:00
    $retro=1;
    if ($data_source =~ /AK/) {
       $model_name_for_metadata = "AK_retro";
    } elsif ($data_source=~ /HRRR/) {
       $model_name_for_metadata = "HRRR_retro";
    } else {
       $model_name_for_metadata = "RAP_retro";
    }
    $startSecs = $hours_ago;
    $endSecs = abs($ARGV[$i_arg++]);
...
unless($retro) {
    $model_name_for_metadata = $data_source;
    my $time = time();
    # get on hour boundary
    $time = $time - $time%3600 - $hours_ago*3600;
    #print "time is $time\n";
    @valid_times = ($time,$time-1*3600,$time-3*3600,$time-8*3600);
}

print("valid times for $data_source are @valid_times\n");
...
my $file_size = -s $file;
if($file_size == 0) {
    print "FILE EMPTY $data_source $fcst_len h fcst valid at $valid_time\n";
    next;
}
# get grid details
