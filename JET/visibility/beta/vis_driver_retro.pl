...
# get fcst_lens, regions, and thresholds for all retros
# so each retro needn't be entered into regions_per_model, fcst_lens_per_model, and thresholds_per_model.
   my $retro_temp = "";
    if ($model =~ /AK/) {
       $retro_temp = "AK_retro";
    } elsif ($model =~ /HRRR/) {
...
for(my $valid_time = $time;$valid_time <= $endSecs;$valid_time += 3600) {
