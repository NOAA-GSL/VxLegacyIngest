...
void write_vis (STATION *sta[],int n_stations,
  ...
  /* open temporary file */
  pid = getpid();
  sprintf(tmp_file_name,"/home/amb-verif/visibility/tmp/%d_vis_data.tmp",pid);
  ...
  sprintf(query,
	  "load data concurrent local infile '%s' replace into table %s columns terminated by ',' (madis_id,time,fcst_len,vis100)",tmp_file_name,model);
  printf("%s\n",query);  
