...
!
         ...
         real(8) :: drfdat(mxmn,mxlv),tpc(mxlv,20),vtcd
         ...
         integer        :: i,k,kk,kk2,kk3,iret,iobs,ivar,iret2,iret3,iretd,j
         logical        :: if_duplicate,ifnotused,is_vtemp
         ...
         call ufbqcd(unit_in,'VIRTMP',vtcd)
           ...
           msg_report: do while (ireadmg(unit_in,subset,idate) == 0)
             ...
             sb_report: do while (ireadsb(unit_in) == 0)
               ...
               call ufbint(unit_in,obs,mxmn,mxlv,iret,obstr)  ! obs is a 2d array (var,level)
               call ufbint(unit_in,oer,mxmn,mxlv,iret,oestr)
               call ufbint(unit_in,qcf,mxmn,mxlv,iret,qcstr)
               call ufbevn(unit_in,tpc,1,mxlv,20,iret,'TPC')
               !if (tpc(k,j)==vtcd) then it is virtual temperature
...
!  convert it to above surface level
               ...
               ! convert virtual temp to sensible temp, if needed
               do k=1,iret
                  is_vtemp=.false.
                  do j=1,20
                     if (tpc(k,j)==vtcd) then
                        ...
                        !write(*,*) 'found flag! ' ,c_sid,j,obs(1,k),obs(2,k),obs(3,k)
                        !<if not missing, convert obs(3,k) to sensible temperature>
                        if(abs(obs(1,k))  < 1e5 .and. abs(obs(2,k)) < 1e5 .and. abs(obs(3,k)) < 1e5) then
                           obs(3,k) = sensible_temp(obs(1,k),obs(2,k),obs(3,k), c_sid)
                        else
                           !write(*,*) 'MISSING VALUES FOR', c_sid,j,obs(1,k),obs(2,k),obs(3,k)
                        endif
                        exit
                      endif
                  enddo
                  if (.not. is_vtemp) then
                     !print *,'no vtemp for ntb, level', ntb,c_sid,k,obs(1,k),obs(2,k),obs(3,k)
                  endif
               enddo
               !write(*,*) 'end of loop over levels for ntb= ',ntb,c_sid
               !
...
!         this%n_alloc=iobs
      ...
      real function sensible_temp(p,dp,Tv,c_sid)
        ! generates the sensible temperature from a virtual temperature, both in Celsius
        real(8), intent(in) :: p,dp,Tv
        character(8), intent(in)   :: c_sid
        ...
        real :: vp,mr
         ! use Teten's Formula to get vapor pressure (expects dp (dewpoint) in CELSIUS)
        vp = 6.11*(10**((7.5*dp)/(237.3+dp))) 
        mr = (eps*vp)/(p-vp)
        ! from https://glossary.ametsoc.org/wiki/Virtual_temperature
        sensible_temp = (Tv+tzero)*(1. + mr)/(1. + mr/eps) - tzero
        !print *,'station, Ts,Tv,diff,p: ',c_sid,sensible_temp,Tv,sensible_temp-Tv,p
